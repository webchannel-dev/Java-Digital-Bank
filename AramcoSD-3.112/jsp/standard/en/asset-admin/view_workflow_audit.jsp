...
<div class="copy">
	...
	<h2><bright:cmsWrite identifier="subhead-asset-approval-messages" filter="false"/> - <bright:cmsWrite identifier="item" case="mixed" /> (<bean:write name='assetId' />)</h2>
	...
	<table class="usage" cellspacing="0" cellpadding="0" border="0" style="margin:0;">
		...
		<logic:iterate name="workflowAuditForm" property="approvalAudit" id="entry">
			...
			<tr>
				...
				<c:if test="${userprofile.isAdmin}">
					<td>
						<center>[<a href="deleteWorkflowAuditEntry?id=<bean:write name="entry" property="id"/>&assetId=<bean:write name="entry" property="entityId"/>">X</a>]</center>
					</td>
				</c:if>
	...
	<input type="button" value="<bright:cmsWrite identifier='link-print' filter='false' />" class="button js-enabled-show" id="closeButton" onclick="window.print();" />

	<logic:notEmpty name="workflowAuditForm" property="approvalAudit">
		<c:if test="${userprofile.isAdmin}">
			<a href="deleteWorkflowAuditEntry?id=-1&assetId=<bean:write name="entry" property="entityId"/>">Delete all messages &raquo;</a>
		</c:if>
	</logic:notEmpty>
