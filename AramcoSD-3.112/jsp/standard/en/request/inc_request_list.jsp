			...
			<tr>
				...
				<c:if test="${showStatus}">
					<td>
						...
						<bean:write name="aRequest" property="workflowInfo.state.description" />
					</td>
				</c:if>
				<td>[<a href="viewRequest?id=<c:out value='${aRequest.id}' /><c:out value='${extraParams}' />" /><bright:cmsWrite identifier="link-view" /></a>]
					<c:if test="${(aRequest.workflowInfo.stateName == 'requires-approval') && (userprofile.isAdmin || aRequest.requestUser.id == userprofile.userId) }">
						[<a href="viewEditRequest?id=<c:out value='${aRequest.id}' /><c:out value='${extraParams}' />" /><bright:cmsWrite identifier="link-edit" /></a>]
					</c:if>
					<c:if test="${userprofile.user.id == requestDetails.requestUser.id || userprofile.isAdmin}">
						[<a href="deleteRequest?id=<c:out value='${aRequest.id}' /><c:out value='${extraParams}' />" onclick="return confirm('Are you sure you want to delete this request?');"/><bright:cmsWrite identifier="link-delete" /></a>]
					</c:if>
				</td>
			</tr>
		</logic:iterate>
