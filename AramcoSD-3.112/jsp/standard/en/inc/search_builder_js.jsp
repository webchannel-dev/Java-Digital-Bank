<script type="text/javascript">
	...
	*/
	...
	var iText=0, iList=1, iDate=2, iNumeric=3, iSpatialAreaBound=4;
	...
	// Build the operator option lists for all of the attribute types
	...
	aOperatorOptions[iSpatialAreaBound] = [new Option("<bright:cmsWrite identifier="snippet-op-equal-to" filter="false"/>",3)];
	...
	<logic:iterate name="attributes" id="attribute">
		<c:choose>
			...
			<c:when test="${attribute.isSpatialArea}">
				addAttributeType(iSpatialAreaBound,'<c:out value="${attribute.id}"/>');
			</c:when>
			<c:otherwise>
	...
	*/
	...
	{
		// Parse ctrl.value in case it is a spatial area subfield
		var ctrlValue = ctrl.value;
		var id = ctrlValue;
		var iIndex = ctrlValue.indexOf('_');
		if (iIndex > 0)
		{
			id = ctrlValue.substring(0, iIndex);
		}
		
		var operator = document.getElementById('operator'+index);
		var newOptions = getOptionsForAttribute(id);
		var iType = getTypeForAttributeId(id);
		...
		{
			var value = '';
			var ctrlType = getTypeForAttributeId(id);
			...
			var myAjax = new Ajax.Updater('valueContainer'+index, 'viewSearchBuilderAttributeValue?id=' + id + "&index=" + index + "&value=" + value, {
				method: 'get',
				evalScripts: true
			});
		}
