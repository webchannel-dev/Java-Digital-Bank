...
<body id="manageLightboxesPage">
		...
		<logic:iterate name="userprofile" property="assetBoxes" id="assetbox" indexId="index">
			<tr>
				<c:if test="${assetbox.shared}">
					<td><bean:write name="assetbox" property="originalName" filter="false"/> (<bean:write name="assetbox" property="assetBoxSize" filter="false"/>)</td>
					<c:if test="${sharedLightboxes && numAssetBoxes!=numOwnAssetBoxes}">
						<td>
					...
					<td class="action">
						[<a href="viewEditAssetBox?id=<c:out value="${assetbox.id}"/>&name=<c:out value="${assetbox.name}"/>&shared=true&publicAvailability=<c:out value="${assetbox.publicAvailability}"/>"><c:choose><c:when test="${!sharedLightboxes}"><bright:cmsWrite identifier="link-edit" filter="false"/></c:when><c:otherwise><bright:cmsWrite identifier="link-rename" filter="false"/></c:otherwise></c:choose></a>]
					</td>
					...
					<td class="action">-</td>
					<td class="action"> 
						[<a href="removeSharedAssetBox?id=<c:out value="${assetbox.id}"/>" onclick="return confirm('<bright:cmsWrite identifier="js-confirm-remove-lightbox" filter="false" />');">remove share</a>]
					</td>
				</c:if>
				<c:if test="${!assetbox.shared}">
					<td><bean:write name="assetbox" property="name" filter="false"/> (<bean:write name="assetbox" property="assetBoxSize" filter="false"/>)</td>
					<c:if test="${sharedLightboxes && numAssetBoxes!=numOwnAssetBoxes}">
						<td><span class="disabled">n/a</span></td>
						<td>me</td>
						<td>yes</td>
					...
					<td class="action"> 
						[<a href="viewEditAssetBox?id=<c:out value="${assetbox.id}"/>&name=<c:out value="${assetbox.name}"/>&shared=false&publicAvailability=<c:out value="${assetbox.publicAvailability}"/>"><bright:cmsWrite identifier="link-edit" filter="false"/></a>]
					</td>
					<c:if test="${multipleLightboxes}">
						<td class="action"> 
							[<a href="viewCopyAssetBox?id=<c:out value="${assetbox.id}"/>&name=<c:out value="${assetbox.name}"/>">copy</a>]
	...
	<br />
	<c:if test="${numAssetBoxes > 1}"><p><a href="../action/alphabetiseAssetBoxes"><bright:cmsWrite identifier="link-alphabetise-lightboxes" filter="false" /></a></p></c:if>
	<br/>
