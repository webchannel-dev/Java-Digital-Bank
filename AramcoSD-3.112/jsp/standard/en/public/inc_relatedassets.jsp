...
<%@ taglib uri="/WEB-INF/fmt.tld" prefix="fmt" %>
		...
		<c:if test="${showRelatedAssets}">
			<c:choose>
				<c:when test="${!assetForm.asset.thisCurrentVersion}">
					<p><bright:cmsWrite identifier="snippet-related-latest-version" filter="false" /></p>
				</c:when>
				<c:otherwise>
					...
					<c:if test="${showChildren}">
						...
						<c:if test="${assetForm.userCanUpdateAsset}">
							...
							<c:if test="${userprofile.canUploadAtAll}">
								<c:if test="${assetForm.asset.entity.hasNonExtensionAssetChildren && not empty assetForm.userEditableChildAssetEntityIds}"><a href="../action/viewUploadAssetFile?parentId=<bean:write name="assetForm" property="asset.id"/>&entityRestrictions=<bean:write name="assetForm" property="userEditableChildAssetEntityIds"/>"><bright:cmsWrite identifier="link-add-single" filter="false"/> <bean:write name="assetForm" property="asset.entity.childRelationshipToName" filter="false"/></a> |
