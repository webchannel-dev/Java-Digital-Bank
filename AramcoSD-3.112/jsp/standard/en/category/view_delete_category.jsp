				...
				<logic:iterate name="deleteCategoryForm" property="flatCategoryList.categories" id="item" indexId="i">
					<logic:notEqual name='item' property='depth' value='0'>
						<div style="margin-left: <c:out value="${12 * item.depth}"/>px;">
							...
							<input type="radio" class="checkbox" id="moveto<c:out value='${item.id}'/>" name="categoryIdToMoveTo" value="<c:out value='${item.id}'/>" <c:if test="${item.id == deleteCategoryForm.categoryIdToDelete}">disabled="disabled"</c:if>/> <label for="moveto<c:out value='${item.id}'/>" class="after <c:if test='${item.id == deleteCategoryForm.categoryIdToDelete}'>disabled</c:if>"><bean:write name="item" property="name" filter="false"/></label>
