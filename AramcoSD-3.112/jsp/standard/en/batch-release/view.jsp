...
<body id="adminPage"> 
		...
		<c:choose>
			...
			<c:when test="${empty batchReleaseForm.pdfGenerationStatus && batchReleaseForm.assetCount > 0}">
				<div class="group first"  style="float:right">
					<a href="generateBatchReleasePDF?brId=<bean:write name='batchReleaseForm' property='id'/>&name=<bean:write name='batchReleaseForm' property='name'/>&<c:out value='${forwardParams}' />" class="linkButton noRightMargin"><bright:cmsWrite identifier="link-print-pdf" filter="false" /></a>
				</div>
			</c:when>
			<c:otherwise>
				...
				<c:if test="${batchReleaseForm.pdfGenerationStatus == 'Completed'}">
					<div class="group first" style="float:right">	
						<bright:cmsWrite identifier="snippet-indesign-pdf-status" />:
						<bean:write name="batchReleaseForm" property="pdfGenerationStatus" />  
						<a href="downloadBatchReleasePDF?brId=<bean:write name='batchReleaseForm' property='id'/>&pdfFileName=<bean:write name='batchReleaseForm' property='pdfFileName'/>&userPdfFileName=<bean:write name='batchReleaseForm' property='userPdfFileName'/>" class="linkButton noRightMargin"><bright:cmsWrite identifier="button-download" /></a>
					</div>				
				</c:if>
				<c:if test="${batchReleaseForm.pdfGenerationStatus == 'Failed'}">
					<div class="group first" style="float:right">
						...
						<c:if test="${batchReleaseForm.assetCount > 0 }"> 
							<a href="generateBatchReleasePDF?brId=<bean:write name='batchReleaseForm' property='id'/>&name=<bean:write name='batchReleaseForm' property='name'/>" class="linkButton noRightMargin"><bright:cmsWrite identifier="link-print-pdf" filter="false" /></a> 
						</c:if>
					</div>										
				</c:if>				
			</c:otherwise>
						...
						<c:if test="${showToolbarTransitions}">
							...
							<logic:iterate name="batchReleaseForm" property="currentState.transitionList" id="transition">
								<c:if test="${!transition.hidden && !((batchReleaseForm.currentState.name == 'awaiting-approval' || batchReleaseForm.currentState.name == 'further-approval-required') && hasApproved)}">
									<div class="group">
										<a href="transitionBatchRelease?id=<bean:write name='batchReleaseForm' property='id' />&workflowName=<bean:write name='batchReleaseForm' property='currentState.workflowName'/>&transition=<bean:write name='transition' property='transitionNumber'/>&<c:out value='${forwardParams}'/>" onclick="return confirm('<bright:cmsWrite identifier='snippet-confirm-batch-release-transition' case='sentence' />');" class="linkButton noRightMargin"><bean:write name='transition' property='description' /></a>
									</div>	
								</c:if>
							</logic:iterate>
						</c:if>
			...
			<td class="padded"><bright:cmsWrite identifier="snippet-this-br-contains" filter="false"/> <strong><c:out value="${batchReleaseForm.assetCount}" /></strong> 
				<c:choose> 
					<c:when test="${batchReleaseForm.assetCount == 1}">   
						<bright:cmsWrite identifier="item" />. <a href="viewAssetsInBatchRelease?brId=<bean:write name='batchReleaseForm' property='id' />&amp;searchPage=batchrelease&brUrl=<c:out value='${thisUrlForGet}' />"><bright:cmsWrite identifier="link-view-item" filter="false"/></a>
					</c:when>
					<c:otherwise> 
					     <bright:cmsWrite identifier="items" />. <c:if test="${batchReleaseForm.assetCount > 0}"><a href="viewAssetsInBatchRelease?brId=<bean:write name='batchReleaseForm' property='id' />&amp;searchPage=batchrelease&amp;brUrl=<c:out value='${thisUrlForGet}' />"><bright:cmsWrite identifier="link-view-items" case="sentence" filter="false"/></a></c:if>
					</c:otherwise>
				</c:choose> 
				<c:if test="${batchReleaseForm.assetCount > 0}">  
					...
					<c:when test="${batchReleaseForm.invalid || batchReleaseForm.pending}">
						<c:choose>
							<c:when test="${batchReleaseForm.currentState.name == 'not-ready-for-release'}">
								...
								<a href="validateBatchRelease?brId=${batchReleaseForm.id}&${forwardParams}" class="linkButton noRightMargin"><bright:cmsWrite identifier="link-validate-batch-release" filter="false" /></a>
							</c:when>
							<c:otherwise>
								<c:if test="${batchReleaseForm.invalid}">
									<div class="errorInline" style="float:left; margin-right:1em">
	...
	<c:if test="${batchReleaseForm.invalid && not empty errors}">
		<br />
		<div class="logHeading"><bright:cmsWrite identifier="subhead-error-log" /></div>
