...
<body id="adminPage">
	...
	<form name="editCustomFieldForm" action="saveCustomField" method="get" class="floated">
			...
			<logic:notEmpty name="customFieldForm" property="customField.translations">
				<logic:iterate name="customFieldForm" property="customField.translations" id="translation" indexId="tIndex">
					<logic:greaterThan name="translation" property="language.id" value="0">
						...
						<input type="text" class="text" name="customField.translations[<bean:write name='tIndex'/>].name" maxlength="255" id="label<bean:write name='tIndex'/>" value="<bean:write name="translation" property="name" />" /><br />
		...
		<label for="externalUsers">Show on profile page?:</label>
		<input class="checkbox" type="checkbox" name="customField.showOnProfile" id="showOnProfile" value="1" <c:if test='${customFieldForm.customField.showOnProfile}'>checked</c:if>/>
		<br />
		
		<c:if test="${ ! notUsingActiveDirectory}">
			<tr>
				<label for="customField.ldapProperty">LDAP property ID:</label>
				<html:text name="customFieldForm" styleClass="text" property="customField.ldapProperty" />
				<br/>
			</tr>
		</c:if>
