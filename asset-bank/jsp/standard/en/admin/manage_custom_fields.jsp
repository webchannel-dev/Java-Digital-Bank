		...
		<table cellspacing="0" class="admin" summary="List of Custom Fields">
			...
			<logic:iterate name="customFieldForm" property="customFields" id="field" indexId="index">
				...
				<tr>
					...
					<td>
						<c:choose>
							<c:when test="${field.showOnProfile}">
								yes
							</c:when>
							<c:otherwise>
								no
					...
					<c:if test="${ ! notUsingActiveDirectory}">
						<td>
							<bean:write name="field" property="ldapProperty"/>
						</td>	
					</c:if>
		...
		<form name="addCustomFieldForm" action="saveCustomField" method="get">
			...
			<table class="form" cellspacing="0" cellpadding="0">
						...
						<logic:greaterThan name="translation" property="language.id" value="0">
							<tr>
								...
								<td>
									...
									<input type="text" class="text" name="customField.translations[<bean:write name='tIndex'/>].name" maxlength="255" id="label<bean:write name='tIndex'/>" value="<bean:write name="translation" property="name" />" />
								</td>
							</tr>
						</logic:greaterThan>
					</logic:iterate>
				...
				<tr>
					<th>Show on profile page?:</th>
					<td>
						<html:checkbox name="customFieldForm" styleClass="checkbox" property="customField.showOnProfile" value="1" />
					</td>
				</tr>
				<c:if test="${ ! notUsingActiveDirectory}">
					<tr>
						<th>LDAP property ID:</th>
						<td>
