...
<%--
 	We deliberately get the application-url setting (and don't fall back to inferring it from the request by using the
 	<bright:applicationUrl/> tag) because this is used to determine whether the slideshow can be refreshed in the
 	background, which only works if the application-url setting is set because the background thread doesn't have a
 	request that it can use to find out the application URL if the setting is not set
--%>
<bright:applicationSetting id="applicationUrlSetting" settingName="application-url"/>
...
<body <c:if test="${repurposeSlideshowForm.convertImages}">onload="document.getElementById('conversionDiv').style.display = 'block';"</c:if>>
	...
	<form action="repurposeSlideshow" method="get" id="usageForm" class="floated">
		...
		<label for="rtl"><bright:cmsWrite identifier="snippet-text-rtl" /></label> <html:checkbox name="repurposeSlideshowForm" property="rtl" styleId="rtl" /><br />
		...
		<c:if test="${type == 'search' && refreshPeriod > 0}">
			<logic:notEmpty name='applicationUrlSetting'>
