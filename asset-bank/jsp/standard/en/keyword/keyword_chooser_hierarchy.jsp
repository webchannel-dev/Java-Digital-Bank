...
<bean:parameter id="filter" name="filter" value="a"/>
	...
	<h1><bright:cmsWrite identifier="heading-keyword-chooser" filter="false"/><c:if test="${categoryAdminForm.categoryName!='Keywords'}"> - <bean:write name='categoryAdminForm' property='categoryName' /></c:if></h1> 
			...
			<logic:equal name="categoryAdminForm" property="root" value="false">
				<strong>
					...
					<logic:iterate name="categoryAdminForm" property="ancestorCategoryList" id="ancestorCategory"> &raquo;
						<a href="keywordChooserHierarchy?categoryId=<bean:write name='ancestorCategory' property='id' />&categoryTypeId=<c:out value='${categoryTypeId}'/>&attributeId=<c:out value='${attributeId}'/>"><bean:write name="ancestorCategory" property="name" /></a>
					</logic:iterate>
						...
						<tr>
							...
							<td style="padding-right:0;">
								<c:if test="${keywordAnscestorsSelectable || keyword.numChildCategories==0}">
									<a href="#" name="addbutton" onclick="addKeyword('<bean:write name='attributeId' />', '<bean:write name='keyword' property='fullNameWithEscapedQuotes'  />', '<bean:write name='keywordDelimiter' />'); return false;" id="<bean:write name='keyword' property='fullName' />">[<bright:cmsWrite identifier="link-add" filter="false"/>]</a>&nbsp;
								</c:if>
								<c:if test="${keyword.numChildCategories>0}">
