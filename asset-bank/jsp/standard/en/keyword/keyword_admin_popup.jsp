...
<div>
		...
		<logic:equal name="categoryAdminForm" property="root" value="false">
			<strong>
				...
				<logic:iterate name="categoryAdminForm" property="ancestorCategoryList" id="ancestorCategory"> &raquo;
					<a href="viewKeywordAdminPopup?categoryId=<bean:write name='ancestorCategory' property='id' />&categoryTypeId=<bean:write name='categoryTypeId'/>"><bean:write name="ancestorCategory" property="name" /></a>
				</logic:iterate>
	...
	<h3><em><bean:write name='currentCategoryName' /></em> <bright:cmsWrite identifier="keyword-nodes" filter="false" />:</h3>
	...
	<logic:equal name="categoryAdminForm" property="subCategoryListIsEmpty" value="true">
		...
		<c:if test="${not empty filter && filter!='all'}">
			<p><bright:cmsWrite identifier="snippet-no-keywords-for-letter" filter="false" /> <bright:write name="filter" case="upper" /></p>
		</c:if>
	</logic:equal>
	
	<div class="hr"></div>	
	
	<h3><bright:cmsWrite identifier="snippet-add-keyword-to" filter="false" /> <em><bean:write name='currentCategoryName' /></em>:</h3>
