...
<bright:applicationSetting id="useSecurityQuestions" settingName="use-security-questions" />
<bright:applicationSetting id="showReceiveAlertsOnProfile" settingName="show-receive-alerts-on-profile" />
...
<body id="profilePage">
	...
	<c:if test="${!changeProfileForm.user.remoteUser}">
		 <bright:cmsWrite identifier="profile-change-password" filter="false"/><c:choose><c:when test="${useSecurityQuestions}"><bright:cmsWrite identifier="profile-change-security-questions" filter="false"/></c:when><c:otherwise>.</c:otherwise></c:choose>
	</c:if>
	
	<c:if test="${canRequestPermissionsUpdate}">
		<c:choose>
			<c:when test="${!changeProfileForm.user.remoteUser}">
				or
			</c:when>
			<c:otherwise>
				You can also
	...
	<html:form action="/changeProfile" method="post">
		...
		<table class="form" cellpadding="0" cellspacing="0" summary="Form for changing your user profile">
			...
			<c:set var="changeProfilePage" value="true" scope="request"/>
		...
		<c:choose>
			<c:when test="${showReceiveAlertsOnProfile}">
				<table cellspacing="0" class="form" summary="Form for user permissions">
				<tr>
					<th><label for="receiveAlerts"><bright:cmsWrite identifier="label-user-receive-alerts" /></label></th>
					<td>
						<html:checkbox name="userForm" property="user.receiveAlerts" styleClass="checkbox" styleId="receiveAlerts" />
					</td>
					<td class="padded">
						<bright:cmsWrite identifier="label-user-receive-alerts-note" />
					</td>
				</tr>		
				</table>
			</c:when>
		...
		<input type="submit" id="submitButton" class="button flush floated" value="<bright:cmsWrite identifier="button-save" filter="false" />" /> 
