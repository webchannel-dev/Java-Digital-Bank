...
<bright:applicationSetting settingName="attribute-exclusion-multi-selects-require-all" id="multiSelectsRequireAll" />
...
<body id="adminPage">
	...
	<html:form action="saveGroupAttributeExclusion" method="post" >
		...
		<input type="hidden" name="pageSize" value="<c:out value="${param.pageSize}"/>"/>
			...
			<c:if test="${multiSelectsRequireAll}">
				<p>Your Asset Bank is setup such that multi-select attributes (option lists and checklists) will require the assets to have all values selected below checked (per attribute) before the asset will be excluded.</p>
			</c:if>
			...
			<table cellspacing="0" class="form" summary="Table of attribute exclusions"  width="100%">
				...
				<%-- Only allow exclusions on dropdown lists, checklists and optionlists --%>
				<c:if test="${attribute.isDropdownList || attribute.isOptionList || attribute.isCheckList}">
				<tr>
					<th>
					...
					<td style="padding-top:2px;">
						...
						<logic:iterate name="attribute" property="listOptionValues" id="attValue" indexId="opIndex">
							...
							<tr>
								...
								<td>&nbsp;<bean:write name="attValue" property="value"/></td>
							</tr>
						</logic:iterate>
						</table>
					</td>
