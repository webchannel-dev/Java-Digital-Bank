...
<body id="adminPage">
	...
	<logic:empty name="saveAction">
		...
		<input type="hidden" name="searchCriteria.forename" value="<bean:write name="userForm" property="searchCriteria.forename" />">
		<input type="hidden" name="searchCriteria.surname" value="<bean:write name="userForm" property="searchCriteria.surname" />"/>
		<input type="hidden" name="searchCriteria.username" value="<bean:write name="userForm" property="searchCriteria.username" />"/>
		<input type="hidden" name="searchCriteria.emailAddress" value="<bean:write name="userForm" property="searchCriteria.emailAddress" />"/>
		<html:hidden name="userForm" property="searchCriteria.groupId"/>
		<input type="hidden" name="oldUsername" value="<bean:write name="userForm" property="oldUsername" />"/>
		...
		<table cellspacing="0" class="form" summary="Form for user details">
				...
				<tr>
					...
					<td style="padding-top: 10px;" colspan="2">
						<logic:equal name="userForm" property="user.remoteUser" value="true">
							<bean:write name="directoryName" />
						</logic:equal>
						<logic:equal name="userForm" property="user.remoteUser" value="false">
							Local (not <bean:write name="directoryName" />)
						</logic:equal>
					</td>
				</tr>	
			</c:if>
			...
			<tr>
				<c:choose>
					<c:when test="${!userForm.user.remoteUser && (userprofile.isAdmin || !orgUnitAllUsers)}">	
						...
						<td>
							<input type="text" class="text" name="user.username" maxlength="100" size="16" value="<bean:write name="userForm" property="user.username" />"/>			
						</td>
					</c:when>
					<c:otherwise>
			...
			<c:if test="${userForm.user.remoteUser && not empty user.displayName}">
				<tr>
					...
					<td class="padded">
						<bean:write name="userForm" property="user.displayName" />
					</td>
				</tr>	
			</c:if>
			...
			<c:choose>
				<c:when test="${!userForm.user.isAdmin || (userprofile.isAdmin || !orgUnitAllUsers)}">	
				<tr>
					<th><label for="expiry"><bright:cmsWrite identifier="label-user-expiry-date" filter="false"/></label></th>
					<td colspan="2">
						<input type="text" class="text date small" id="expiry" name="expiryDate" size="20" value="<bean:write name="userForm" property="expiryDate" />"/>	
				...
				<c:otherwise>
					<html:hidden name="userForm" property="expiryDate" />
				</c:otherwise>
			</c:choose>
		...
		<c:choose>
			...
			<c:otherwise>
				<html:hidden name="userForm" property="user.isSuspended" />
				<html:hidden name="userForm" property="user.hidden" />
			</c:otherwise>
		</c:choose>
