	...
	<script type="text/javascript">
		...
		$j(function () {
			...
			<c:if test="${usernameIsEmailaddress}"> 
				// if the user has left username field empty then populate it with email address before 
				// submitting the form
				$j('#userForm').submit(function() {
					if(!$j('#username').val() && $j('#email').val()) {
						$j('#username').val($j('#email').val());
					}
				});
...
<body id="adminPage">
	...
	<html:form action="addUser" method="post" onsubmit="return doConfirmation();">
		...
		<table cellspacing="0" class="form" style="width: auto;" summary="Form for user details">
			...
			<tr>
				<th><label for="username">Username:<c:if test="${!usernameIsEmailaddress}"><span class="required">*</span></c:if></label></th>
				<td <c:if test="${!usernameIsEmailaddress}"> colspan="2"</c:if>>
					<input type="text" class="text" name="user.username" id="username" maxlength="100" size="25" value="<bean:write name="userForm" property="user.username" />"/> 
				</td>
				<c:if test="${usernameIsEmailaddress}"> 
					<td class="note">Leave empty to use email address (recommended)</td>
				</c:if>
			</tr>
			...
			<c:if test="${!forceRemoteAuthentication}">
				<tr>
					...
					<td rowspan="2" class="note">Leave empty for a password to be automatically generated and emailed to the new user</td>												
				</tr>
				<tr>
			...
			<tr>
				...
				<td colspan="2">
					...
					<span class="note">(<c:out value="${dateFormatHelpString}" />) - Leave empty for never</span>			
