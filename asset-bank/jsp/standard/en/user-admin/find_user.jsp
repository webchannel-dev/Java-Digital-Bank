...
<bright:applicationSetting id="findUsersMaxResults" settingName="find-users-max-results"/>
...
<body id="adminPage">
	...
	<c:if test="${userForm.emptyResult || not empty userForm.users}">
			<div class="hr"></div>
			<!-- Needs Designers hand -->
			<bean:size id="numberOfUsers" name="userForm" property="users"/>
			
			<div class="toolbar">
				<h2><c:out value="${numberOfUsers}"/> user<c:if test="${numberOfUsers!=1}">s</c:if> found</h2>
			...
			<c:if test="${numberOfUsers == findUsersMaxResults}">
				<div class="warning">Too many results to display, please refine your search! Showing the first <strong><c:out value="${findUsersMaxResults}"/></strong> results: </div>
			</c:if>	
	...
	<logic:notEmpty name="userForm" property="users">
			<table cellspacing="0" class="list highlight noTopMargin" summary="List of users" >		
				<thead>
				<tr>
				...
				<logic:iterate name="userForm" property="users" id="user">
					<tr>
						...
						<c:if test="${userForm.showExpiryDate}">
							<td>
								<bean:write name="user" property="expiryDate" />
							</td>
						</c:if>
						<td class="action">
