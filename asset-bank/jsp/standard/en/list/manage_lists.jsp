								...
								<td class="action">
									<c:choose>
										<c:when test="${not listItem.created}">
											[<a href="viewEditListItem?id=<bean:write name='listItem' property='identifier'/>&listOrder=2&languageId=<bean:write name='listItem' property='language.id'/>&url=/action/contentSearch">edit</a>]
										</c:when>
										<c:otherwise>
											<span style="color: black">[<a href="viewEditListItem?id=<bean:write name='listItem' property='identifier'/>&listOrder=2&languageId=<bean:write name='listItem' property='language.id'/>&url=/action/contentSearch">add</a>]</span>
										</c:otherwise>
									</c:choose></td>
								<td class="action">
									<c:if test="${not listItem.cannotBeDeleted || not listItem.language.default}">
										<c:choose>
											<c:when test="${not listItem.created}">
												[<a href="deleteListItem?id=<bean:write name='listItem' property='identifier'/>&listid=<bean:write name='listItem' property='listId'/>&languageId=<bean:write name='listItem' property='language.id'/>&listOrder=2" onclick="return confirm('Are you sure you want to delete this content item?');">X</a>]
											</c:when>
											<c:otherwise>
