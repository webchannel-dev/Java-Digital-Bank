...
<head>
 	...
 	function updateDefaultValues(attributeTypeId)
	{
		 ...
		 *
		 * We dont change the maxSize if it is not the default so that if the
	...
	function showMaxDisplay (value)
	{
		...
		{
			changeStyleDisplay('extendableListField', "block");
		}
...
<body id="adminPage" onload="showRelevantFieldsOnLoad();">
	...
	<html:form action="saveAttribute" method="post">
		...
		<table cellspacing="0" border="0" class="form" summary="Form for increment details" id="incrementDisplay" style="display: none; margin:0; width: auto;">
			...
			<tr>	
				<th style="width:120px;">
					<label for="enforceUniqueness">Enforce Uniqueness:</label> 
				</th>
				<td>
					<html:checkbox styleClass="checkbox" name="attributeForm" property="attribute.enforceUniqueness" styleId="enforceUniqueness"/>
				</td>
			</tr>
			<tr>	
				<th style="width:120px;">
		...
		<c:if test="${attributeForm.attribute.id==0 || attributeForm.attribute.isTextfield || attributeForm.attribute.isTextarea || attributeForm.attribute.isHyperlink || attributeForm.attribute.isOptionList || attributeForm.attribute.isCheckList || attributeForm.attribute.isDropdownList || attributeForm.attribute.isDatepicker || attributeForm.attribute.isKeywordPicker}">
					...
					<logic:iterate name="attributeForm" property="attribute.translations" id="translation" indexId="tIndex">
						<logic:greaterThan name="translation" property="language.id" value="0">
							<tr>
								...
								<td>
									<input type="text" class="text" name="attribute.translations[<bean:write name='tIndex'/>].altText" maxlength="255" id="default<bean:write name='tIndex'/>" value="<bean:write name="translation" property="altText" />" />
								</td>
							</tr>
						</logic:greaterThan>
					</logic:iterate>
