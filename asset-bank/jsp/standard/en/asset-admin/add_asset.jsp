...
<body id="importPage" onload="setDescSelectedCategories(); setPermSelectedCategories(); setCatIdsFields(); showHideAgreementType(); syncAgreementPreviewButton();">
	...
	<c:if test="${not empty assetForm.asset.entity.name}">
		(<bean:write name="assetForm" property="asset.entity.name" />)
	</c:if>	
	</h1> 


	<c:choose>
		<c:when test="${!empty assetForm.previewImageFile}">
			<p>
		...
		<c:when test="${assetForm.previewConversionFailed}">
			<div class="warning">
				<bright:cmsWrite identifier="upload-conversion-warning" />
			</div>
		</c:when>
	</c:choose>
	...
	<html:form enctype="multipart/form-data" action="addAsset" styleId="updateForm" method="post">
		...
		<html:hidden name="assetForm" property="previewConversionFailed"/>
		...
		<html:hidden name="assetForm" property="fulfilledRequest.id"/>
		<html:hidden name="assetForm" property="fulfilledRequest.message"/>
