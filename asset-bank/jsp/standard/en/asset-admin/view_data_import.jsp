			...
			<c:when test="${userprofile.batchImportTimestamp gt 0}">
					<div class="confirm noIcon">
						...
						<c:if test="${!hideIncompleteAssets || !allowSearchByCompleteness}">
							<c:if test="${importForm.numIncompleteAssets==0 && !importForm.hideCompletenessMessage}">
								<h3 class="confirm" style="margin-top:1em"><bright:cmsWrite identifier="subhead-all-uploaded-complete-metadata"/></h3>
							</c:if>	
						</c:if>
					</div>
					...
					<c:if test="${importForm.numIncompleteAssets>0 && !importForm.hideCompletenessMessage}">
						<div class="warning">
							...
							<a href="createNewBatchFromImport?incomplete=true&finishedUrl=<c:out value="${thisUrl}"/>" class="multilineButton">
							...
							<a class="multilineButton" href="createNewBulkUpdateFromImport?incomplete=true&cancelUrl=<c:out value="${thisUrl}"/>">
			...
			<div <c:if test="${importForm.isImportInProgress}">class="js-enabled-hide"</c:if>>
				...
				<c:if test="${!importForm.isImportInProgress}">
					<div class="toolbar" style="margin-top:1.5em; margin-bottom:0;">
						...
						<c:set var="numEditableItems"><c:out value="${importForm.numEditableAssets}" /></c:set>
						<h3><bright:cmsWrite identifier="subhead-num-uploaded-items" filter="false" replaceVariables="true" />
						<c:if test="${numEditableItems>0 && numEditableItems<numUploadedItems}"> <bright:cmsWrite identifier="snippet-can-still-edit-x" filter="false" replaceVariables="true" /></c:if>
						</h3>	
						<c:if test="${importForm.numEditableAssets>0}">
							<div class="group">	<a href="createNewBatchFromImport?finishedUrl=<c:out value="${thisUrl}"/>" title="<bright:cmsWrite identifier="snippet-batch-update-summary" filter="false"/>"><bright:cmsWrite identifier="link-batch-update-these-items" filter="false"/></a></div>
								<div class="group"><a href="createNewBulkUpdateFromImport?cancelUrl=<c:out value="${thisUrl}"/>" title="<bright:cmsWrite identifier="snippet-bulk-update-summary" filter="false"/>"><bright:cmsWrite identifier="link-bulk-update-these-items" filter="false"/></a></div>
