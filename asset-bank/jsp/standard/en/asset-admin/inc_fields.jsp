...
<un:useConstants var="attributeConstants" className="com.bright.assetbank.attribute.constant.AttributeConstants" />
								...
								<c:if test="${(assetForm.asset.entity.id==0 || assetForm.asset.entity.allowAssetFiles) && !assetForm.emptyAsset}">
									<c:if test="${!bIsImport}">
									   ...
									   <td></td>
										...
										<c:if test="${assetForm.asset.hasSubstituteFile || (allowImageSubstitutes && assetForm.asset.isImage) || assetForm.previewConversionFailed}">
											<tr>
												<th>
									...
									<c:if test="${!(assetForm.asset.format.rotatable == false) && showRotate && assetForm.asset.typeId == 2 && assetForm.asset.numPages == 1 && (assetForm.asset.entity.id==0 || assetForm.asset.entity.allowAssetFiles)}"> <%-- Images --%>					
										<tr>
					...
					<c:if test="${!attribute.isVisible}">
						...
						<c:choose>
							...
							<c:when test="${attribute.isKeywordPicker}">
								<input type="hidden" name="attribute_<bean:write name='attribute' property='id'/>" value="<logic:notEmpty name="attribute" property="value"><logic:notEmpty name="attribute" property="value.keywordCategories"><logic:iterate name="attribute" property="value.keywordCategories" id="category" indexId="index"><bean:write name="category" property="fullName"/><bean:write name="keywordDelimiter"/></logic:iterate></logic:notEmpty></logic:notEmpty>">
							</c:when>
		...
		<c:if test="${attribute.isVisible}">
			...
			<tr class="sensitivityNotes">
				...
				<td>
					<div id="sensitivityNotes">
						...
						<c:if test="${showDefaultLanguageLabel}">
							<bright:cmsWrite identifier="term-for-default-language" filter="false"/>:<br/>
						</c:if>
			...
			<c:if test="${assetForm.editableLanguagesCount>0}">
				<c:if test="${defaultNotRealLanguage}">
					<tr class="sensitivityNotes"><td colspan="2">&nbsp;</td><c:if test="${bIsBulkUpdate}"><td>&nbsp;</td><td>&nbsp;</td></c:if><td><bright:cmsWrite identifier="snippet-language-specific-overrides"/>:</td></tr>
				</c:if>
			...
			<tr class="sensitivityNotes spacer"><td colspan="4"></td></tr>
			<script type="text/javascript">
	...
	<c:choose>
		<c:when test="${ userprofile.isAdmin && numPromoted gt 0 }">
		<tr>
			<th class="checkbox">
		...
		<c:otherwise>
			<html:hidden name="assetForm" property="asset.isPromoted" />
		</c:otherwise>
	</c:choose>

		<%-- Checkbox for add images to featured set (admin only) --%>
	<c:choose>
		<c:when test="${ userprofile.isAdmin && (assetForm.asset.isImage || assetForm.asset.isVideo) && assetForm.asset.surrogateAssetId<=0 && iFeaturedImageWidth gt 0}">
		<tr>
			<th class="checkbox">
		...
		<c:otherwise>
			<html:hidden name="assetForm" property="asset.isFeatured" />
		</c:otherwise>
	</c:choose>	
	...
	<c:if test="${!hidePermissionCategories || !hideDescriptiveCategories}">
		<c:if test="${(anyDescriptiveCategories && assetForm.areCategoriesVisible) || (!hidePermissionCategoriesOnEdit && !bIsBulkUpdate)}">
			<div class="hr"></div>
		</c:if>
	</c:if>
