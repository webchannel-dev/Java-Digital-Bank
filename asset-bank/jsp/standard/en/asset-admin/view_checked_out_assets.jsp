...
<body id="adminPage">
	...
	<c:if test="${userprofile.isAdmin}">
		<div class="adminTabs">
			<h2 class="current"><bright:cmsWrite identifier="tab-my-checked-out" filter="false" case="mixed"/></h2>
			<h2><a href="viewAllCheckedOutAssets"><bright:cmsWrite identifier="tab-all-checked-out" filter="false" case="mixed"/></a></h2>
    ...
    <c:when test="${empty checkedOutAssetsForm.checkedOutAssets}">
		...
		<h2><bright:cmsWrite identifier="subhead-my-checked-out-items-empty" filter="false" /></h2>

    </c:when>
    <c:otherwise>
	
		<h2><bright:cmsWrite identifier="subhead-my-checked-out-items" filter="false" /></h2>
			...
			<tbody>
			    <logic:iterate name="checkedOutAssetsForm" property="checkedOutAssets" id="asset" >
					<tr>
						...
						<td>
							[<a href="viewAsset?id=<c:out value="${asset.id}" />"><bright:cmsWrite identifier="link-view" filter="false"/></a>] 
						</td>
					</tr>
				</logic:iterate>
			</tbody>
