...
<body id="detailsPage">
	...
	<form action="../action/saveRelationshipDescriptions" method="post">
		...
		<input type="hidden" name="addingFromBrowseCatId" value="<c:out value='${relationshipDescriptionForm.addingFromBrowseCatId}' />" />
		<input type="hidden" name="addingFromBrowseTreeId" value="<c:out value='${relationshipDescriptionForm.addingFromBrowseTreeId}' />" />
		...
		<c:choose>
			...
			<c:otherwise>
				<c:choose>
					<c:when test="${relationshipDescriptionForm.addingFromBrowseTreeId > 0 && relationshipDescriptionForm.addingFromBrowseCatId > 0 && categoryExplorerSetting == 'none'}">
						<a class="cancelLink" href="../action/browseItems?categoryId=<c:out value='${relationshipDescriptionForm.addingFromBrowseCatId}' />&categoryTypeId=<c:out value='${relationshipDescriptionForm.addingFromBrowseTreeId}' />">Skip this step</a>
					</c:when>
					<c:when test="${relationshipDescriptionForm.addingFromBrowseTreeId > 0 && relationshipDescriptionForm.addingFromBrowseCatId > 0 && categoryExplorerSetting != 'none'}">
						<a class="cancelLink" href="../action/viewHome">Skip this step</a>
					</c:when>
					<c:otherwise>
