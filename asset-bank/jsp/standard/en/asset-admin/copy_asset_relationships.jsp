							...
							<tr>
								...
								<td style="width: 48%;">				
									...
									<%@include file="../inc/result-video-audio-icon.jsp"%>
										...
										<label for="should<bean:write name='identifier' />" class="wrapping"><input type="checkbox" class="checkbox" name="should<bean:write name='identifier' />" id="should<bean:write name='identifier' />" value="1" onclick="maintainRelationship(this, <bean:write name='identifier' />)"/> <bright:cmsWrite identifier="label-include-this-item" filter="false"/></label>
										<div id="radios<bean:write name='identifier' />" class="radioGroup">
											<label for="<bean:write name='identifier' />" class="wrapping"><input type="radio" class="checkbox" name="<bean:write name='identifier' />" id="<bean:write name='identifier' />" value="1" checked /> <bright:cmsWrite identifier="label-use-the-original" filter="false"/></label>
											<label for="<bean:write name='identifier' />copy" class="wrapping"><input type="radio" class="checkbox" name="<bean:write name='identifier' />" id="<bean:write name='identifier' />copy" value="2" />  <bright:cmsWrite identifier="label-create-a-copy" filter="false"/></label>
										</div>
								...
								<td <c:if test="${childAssetsSize gt 1}">class="copyAction"</c:if>>
									...
									<c:if test="${myIndex==0}">
										<c:if test="${childAssetsSize gt 1}">
												<a href="#" id="copyValuesLink"><bright:cmsWrite identifier="link-copy-these-values-to-all" filter="false"/></a>
										</c:if>	
									</c:if>	
									&nbsp;
								</td>
