						...
						<c:when test="${attribute.isVisible}">
							<c:choose>
								...
								<c:when test="${attribute.isTextfield || attribute.isTextarea}">
									<tr>
										<th class="checkbox">
											<label for="${attributeConstants.k_sParam_AttributeId}${attribute.id}"><bean:write name="attribute" property="label" /><logic:notEmpty name="attribute" property="label">:</logic:notEmpty> </label>
												...
												<c:set var="language" value="${userprofile.currentLanguage}"/>
													<span style="font-weight: normal;"> 
														...
														<c:if test="${showNativeLanguages && not empty language.nativeName}">
															(<bean:write name="language" property="nativeName" />)
														</c:if>
													</span>
											</c:if>
										</th>
