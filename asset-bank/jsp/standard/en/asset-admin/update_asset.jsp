...
<c:if test="${not empty assetForm.asset.entity.name}">
	<c:set var="pagetitle"><c:out value="${pagetitle}"/> <bean:write name="assetForm" property="asset.entity.name" /></c:set>
	<c:set var="pageheading"><c:out value="${pageheading}"/> <bean:write name="assetForm" property="asset.entity.name" /></c:set>
</c:if>
...
<head>
	...
	<c:if test="${not empty assetAdminJavascriptFile}">
		<script src="<bean:write name="assetAdminJavascriptFile" />" type="text/javascript"></script>
	</c:if>
...
<body onload="<c:if test='${assetForm.areCategoriesVisible}'>setDescSelectedCategories();</c:if> <c:if test='${!hidePermissionCategoriesOnEdit}'>setPermSelectedCategories();</c:if> setCatIdsFields(); showHideAgreementType(); syncAgreementPreviewButton();">
	...
	<html:form enctype="multipart/form-data" action="updateAsset" styleId="updateForm" method="post">
		...
		<logic:notEmpty name="assetForm" property="asset.displayPreviewImageFile.path">
			...
			<%--  Steve 05-Jan-2012: Add case of plain file with video preview as replacement thumbnail --%>	
			<c:if test="${assetForm.asset.isVideo || (assetForm.asset.isPlainFile && !empty assetForm.asset.encryptedEmbeddedPreviewClipLocation)}"> 
   				<p><a href="viewSelectThumbnail?id=<bean:write name='assetForm' property='asset.id'/>&pageNo=0&dirName=<bean:write name='assetForm' property='tempDirName'/>" onclick="if (hasChanged) { return confirm('<bright:cmsWrite identifier="js-confirm-lose-changes" filter="false"/>'); }"><bright:cmsWrite identifier="link-choose-diff-frame" filter="false"/></a></p>
			</c:if>
		...
		<c:if test="${supportMultiLanguage}">
			<p class="infoInline floatRight noBottomMargin "><bright:cmsWrite identifier="snippet-define-languages-profile" filter="false"/></p>
		</c:if>	
		<h2 style="line-height:24px;"><bright:cmsWrite identifier="heading-edit-metadata" filter="false"/></h2>
