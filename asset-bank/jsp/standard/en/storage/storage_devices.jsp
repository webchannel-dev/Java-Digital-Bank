...
<head>
	<title><bright:cmsWrite identifier="company-name" /> | Storage Devices</title> 
...
<body id="adminPage">
	...
	<em>Thumbnails</em> (generated thumbnail and preview images), <em>Repurposed files</em> (files generated for embedding) or <em>System files</em> (inbuilt icons and temporary files used by the application). </p>
	<p>The storage device that comes with a new Asset Bank is called the <em>Default</em> storage device, and contains inbuilt files such as icons. This cannot be deleted but may be locked if you would like to create other devices instead.</p>
	...
	Ensure that there is sufficient available storage space allocated for Asset files, Thumbnails and System files.</p>
	<p>
	<em>Used for</em>: what resources will be stored in the device (System files, Thumbnails or Asset files).<br/>
	<em>Local Path</em>: the filesystem path of the device.<br/>
	...
	<table cellspacing="2" border="0" class="admin compact" summary="List of storage devices">		
		...
		<logic:iterate name="storageDeviceForm" property="devices" id="device" indexId="index">
			<tr>
				<td>
					<bean:write name="device" property="deviceTypeName" />
				</td>
				<td>
					<bean:write name="device" property="name" />
					<c:if test="${device.isDefault}"><em>(default)</em></c:if>
				</td>
				<td>
					<c:choose>
				...
				<td>
					...
					<c:if test="${pathLength>15}">
						<a style="font-weight:normal; cursor: help;" title="<bean:write name="path" />"><bright:write name="path"  maxLength="15" endString="..."/></a>
					</c:if>
					<c:if test="${pathLength<=15}">
