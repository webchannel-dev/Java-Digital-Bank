...
<bright:applicationSetting id="dateTimeFormat" settingName="display-datetime-format" />
...
<body id="adminPage">
	...
	<div class="head">
		<a href="../action/viewAuditReportPage?startDateString=<bean:write name='auditReportForm' property='startDateString'/>&endDateString=<bean:write name='auditReportForm' property='endDateString'/>&ipAddress=<bean:write name='ipAddress'/>&username=<bean:write name='username'/>&includeViewsDownloads=<bean:write name='includeViewsDownloads'/>" class="bold">&laquo; Back to Reports</a>
	</div>

	<h2>Audit Report</h2>
	
	<logic:empty name="auditReportForm" property="auditReportLines">
		<p>There are no audit logs for the date range you provided.</p>
	</logic:empty>
	
	<logic:notEmpty name="auditReportForm" property="auditReportLines">
		<h3>
			<c:choose>
		...
		<table cellspacing="0" class="report" summary="Search report" style="width: 80%;">		
			...
			<logic:iterate name="auditReportForm" property="auditReportLines" id="reportLine" indexId="index">
				<tr>
					<c:if test="${showThumbnails}">
						<td>
					...
					<td>
						<fmt:formatDate value="${reportLine.date}" pattern="${dateTimeFormat}" />
					</td>
					<td>
						<c:choose>
							...
							<c:otherwise>
								<a href="viewAsset?id=<bean:write name="reportLine" property="assetId" />"><bean:write name="reportLine" property="assetId" /></a>
							</c:otherwise>
						</c:choose>
					</td>
					<td>
