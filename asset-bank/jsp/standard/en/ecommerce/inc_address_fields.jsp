	...
	<bright:refDataList id="labelPostcode" componentName="ListManager" methodName="getListItem" argumentValue="label-postcode"/>	
		...
		<logic:notEmpty name="labelAddress1">
			<tr>
				<th valign="top"><label for="address1"><bright:write name="labelAddress1" property="body" /></label><c:if test="${strictValidation}"><span class="required">*</span></c:if></th>
				<td>
					<html:text styleClass="text" styleId="address1" name="checkoutForm" property="shippingAddress.addressLine1" size="16" maxlength="200"/>
				</td>
				<c:if test="${strictValidation}"><input type="hidden" name="mandatory_shippingAddress.addressLine1" value="<bright:cmsWrite identifier="failedValidationForField" filter="false"/> <bright:write name="labelAddress1" property="body" />." /></c:if>
			</tr>		
		</logic:notEmpty>
		...
		<logic:notEmpty name="labelTown">			
			<tr>
				<th valign="top"><label for="town"><bright:write name="labelTown" property="body" /></label><c:if test="${strictValidation}"><span class="required">*</span></c:if></th>
				<td>
					<html:text styleClass="text" styleId="town" name="checkoutForm" property="shippingAddress.town" size="16" maxlength="100"/>
				</td>
				<c:if test="${strictValidation}"><input type="hidden" name="mandatory_shippingAddress.town" value="<bright:cmsWrite identifier="failedValidationForField" filter="false"/> <bright:write name="labelTown" property="body" />." /></c:if>
			</tr>			
		</logic:notEmpty>
		<logic:notEmpty name="labelCounty">			
			<tr>
				<th valign="top"><label for="county"><bright:write name="labelCounty" property="body" /></label></th>
				<td>
		...
		<logic:notEmpty name="labelPostcode">			
			<tr>
				<th valign="top"><label for="postcode"><bright:write name="labelPostcode" property="body" /></label><c:if test="${strictValidation}"><span class="required">*</span></c:if></th>
				<td>
					<html:text styleClass="text" styleId="postcode" name="checkoutForm" property="shippingAddress.postcode" size="16" maxlength="25"/>
				</td>
			</tr>	
			<c:if test="${strictValidation}"><input type="hidden" name="mandatory_shippingAddress.postcode" value="<bright:cmsWrite identifier="failedValidationForField" filter="false"/> <bright:write name="labelPostcode" property="body" />." /></c:if>
		</logic:notEmpty>
