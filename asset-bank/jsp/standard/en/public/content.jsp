...
<c:choose>
	<c:when test="${identifier != ''}">
		...
		<c:set var="identifierForHeading" value="content-page-menu-${index}" />
...
<body <c:if test="${openNav!=''}">id="<bean:write name='openNav' />"</c:if>>
	...
	<c:if test="${showTitle}"><h1 class="underline"><bright:cmsWrite identifier="${identifierForHeading}" /></h1></c:if>
		
	<%-- This custom copy comes from the database (change in the 'Content' area of Admin) --%>

	<bright:cmsWrite identifier="${identifierToLoad}" filter="false" />
