...
<head>
	...
	<logic:equal name="downloadForm" property="conversionInProgress" value="true">
		<meta HTTP-EQUIV="refresh" CONTENT="5;URL=viewAudioConversionStatus<c:if test="${downloadForm.repurpose || downloadForm.email}">?assetId=<bean:write name="downloadForm" property="asset.id"/><c:choose><c:when test="${downloadForm.repurpose}">&repurpose=true</c:when><c:otherwise>&emailAsset=true</c:otherwise></c:choose></c:if>"></meta>
	</logic:equal>
	<logic:equal name="downloadForm" property="conversionInProgress" value="false">
		<meta HTTP-EQUIV="refresh" CONTENT="1;URL=viewAudioConversionStatus?filePath=<bean:write name='downloadForm' property='videoConversionResult.convertedFilename'/>&compressAsset=<bean:write name='downloadForm' property='videoConversionResult.compress'/><c:if test="${downloadForm.repurpose || downloadForm.email}">&assetId=<bean:write name="downloadForm" property="asset.id"/><c:choose><c:when test="${downloadForm.repurpose}">&repurpose=true</c:when><c:otherwise>&emailAsset=true</c:otherwise></c:choose></c:if>">
	</logic:equal>
	...
	<logic:equal name="downloadForm" property="conversionInProgress" value="false">
		<c:choose>
			<c:when test="${!downloadForm.email}">
				<p><bright:cmsWrite identifier="snippet-audio-conversion-finished" filter="false" /></p>
