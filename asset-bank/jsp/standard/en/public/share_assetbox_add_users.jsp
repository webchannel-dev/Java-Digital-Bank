...
<body onLoad="showHideEmailFields();">
	...
	<form action="<c:out value='${findUsersAction}' />" method="post">
		...
		<input type="submit" class="button flush" value="<bright:cmsWrite identifier='link-find' /> &raquo;" />
	...
	<logic:notEmpty name="shareAssetBoxForm" property="users">
		<html:form action="addAssetBoxUsers" method="post">
			...
			<table cellspacing="0" class="admin" summary="Share details">
										...
										<logic:greaterThan name="translation" property="language.id" value="0">
											<tr>
												...
												<td>
													...
													<input type="text" name="emailContent.translations[<bean:write name='tIndex'/>].subject" maxlength="80" size="40" id="subject<bean:write name='tIndex'/>" value="<bean:write name="translation" property="subject" />"/>
												</td>
											</tr>
										</logic:greaterThan>
									</logic:iterate>	
				...
				<tr>
					<td>
						<input type="submit" value="<bright:cmsWrite identifier='button-add-selected-users' />" class="button flush" onclick="return confirm('<bright:cmsWrite identifier="js-confirm-add-users" filter="false"/>');" style="margin-top:0.7em" />
					</td>
				</tr>
			</table>
		</html:form>
