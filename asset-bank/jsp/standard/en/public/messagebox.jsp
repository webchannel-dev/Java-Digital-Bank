	...
	<html:form action="viewAssetBox" method="get">
		<div class="head stripBelow <c:if test="${tabsPresent}">tabsAbove</c:if>">
			...
			<c:if test="${multipleLightboxes && userprofile.numAssetBoxes>1}">
				<bright:cmsWrite identifier="a-lightbox" filter="false" />:
			...
			<c:set var="numAssetsLb" value="${userprofile.assetBox.numAssets}" />
			...
			<c:if test="${userprofile.assetBox.numAssets>0 && userprofile.canClearAssetBox}">
				|
				<a href="../action/clearAssetBox" onclick="return confirm('<bright:cmsWrite identifier="js-confirm-remove-all-lightbox" filter="false" replaceVariables="true" />');"><bright:cmsWrite identifier="link-remove-all" filter="false"/></a>
			</c:if>
	...
	<%-- Other sections --%>
         ...
         <c:if test="${usePriceBands && withPriceBandsSize > 0}">
					...
					<div class="group">
						<strong>
							<c:if test="${!userprofile.assetBox.hasCommercialUsage && useCommercialOptions}">
								...
								&nbsp;
								<bright:writeMoney name="userprofile" property="assetBox.basketTotal.displayAmount" />
								...
								<c:if test="${userprofile.maxDiscount > 0}">
									...
									<br/><bright:cmsWrite identifier="e-total" filter="false"/> <bright:writeMoney name="userprofile" property="totalWithDiscountPriceBands.displayAmount" /><br/><br/>
								</c:if>
							</c:if>								
						</strong>
					</div>
         	...
         	<ul class="lightbox clearfix cart">
         		...
         		<logic:iterate name="assetBoxForm" property="listWithPriceBands" id="assetinlist" indexId="index">
         			...
         			<jsp:include page="../public/inc_assetcart_asset.jsp"/>	 		
         		</logic:iterate>
         ...
         <c:if test="${canDownloadSize + approvedSize > 0}">
			...
			<form action="../action/actionOnSelectedAssets" method="post" id="actionOnSelected">
         	...
         	<h2 class="section"><bean:write name="subheadReadyDownload" /></h2>
				...
				<div class="toolbar js-enabled-show" id="lbTools">
					...
					<div class="group first">
	         		...
	         		<div id="dummyButtons">
	         			<a href="#" class="button" onclick="return false;"><bright:cmsWrite identifier="button-lightbox-download"/></a>	         		
	         			<a href="#" class="button dropButton last" onclick="return false;"><span></span><bright:cmsWrite identifier="button-lightbox-more-actions"/></a>
	         		</div>
         			<div id="dropButtonSelected">
         				
         				<a href="../action/viewDownloadAssetBox" class="button" id="downloadSelected" onclick="return false;"><bright:cmsWrite identifier="button-lightbox-download"/></a>
         				
         				<div class="dropHolder">
         					<a href="#" class="button dropButton last" title="More actions on selected items" onclick="return false;"><span></span><bright:cmsWrite identifier="button-lightbox-more-actions"/></a>
         			
							<ul class="dropOptions">
								<c:if test="${cacheLargeImage}">
									<li><a href="#" onclick="submitLbAction(2)"><bright:cmsWrite identifier="snippet-compare" filter="false"/></a></li>
								</c:if>
								<li><a href="#" onclick="submitLbAction(3)"><bright:cmsWrite identifier="snippet-remove-from-lightbox" filter="false"/></a></li>
								<c:if test="${showBulkUpdate && userprofile.canUpdateAtAll}">
									<li><a href="#" onclick="submitLbAction(5)"><bright:cmsWrite identifier="heading-bulk-update" filter="false"/></a></li>
								</c:if>
								<c:if test="${userprofile.canUpdateAtAll && getRelatedAssets && !assetEntitiesEnabled && userprofile.assetBox.numAssets>1}">
									<li><a href="#" onclick="submitLbAction(6)"><bright:cmsWrite identifier='button-link-assets' filter='false'/></a></li>
								</c:if>
								<c:if test="${enableSlideshow && userprofile.assetBox.containsImages}">
									<li><a href="#" onclick="submitLbAction(8)"><bright:cmsWrite identifier="snippet-view-as-slideshow" filter="false"/></a></li>
								</c:if>
								<li><a href="#" onclick="submitLbAction(9)"><bright:cmsWrite identifier="heading-copy-move"/></a></li>
							</ul>
         		...
         		<div class="group">
						...
						<a href="../action/viewDownloadAssetBox" class="button first"><bright:cmsWrite identifier="button-download-all" filter="false" /></a>
						<c:if test="${showRequestOnCd}">
							<a href="../action/viewRequestAssetBox" class="button"><bright:cmsWrite identifier="button-request-cd" filter="false"/></a>
						</c:if>	
				...
				<c:if test="${approvedSize > 0 }">
					...
					<ul class="lightbox clearfix" id="lightboxUl">
						...
						<logic:notEmpty name="assetBoxForm" property="listCanDownload">
							<c:set var="index_previous" value="${index+1}"/>
						</logic:notEmpty>
         ...
         <c:if test="${!usePriceBands && approvalRequiredSize > 0}">
				...
				<c:choose>
					<c:when test="${ecommerce}">
						<div class="toolbar" >
         				...
         				<h2 class="section">
								<bean:write name="subheadApprovalRequired" />
							</h2>
							<div class="group">
								<strong>
									<c:choose>
									...
									&nbsp;
									<bright:writeMoney name="userprofile" property="totalWithDiscount.displayAmount" /><c:if test="${userprofile.maxDiscount > 0}"> (inc. <bean:write name="userprofile" property="maxDiscount"/>% discount)</c:if>
								</strong>
							</div>
							<div class="group">
								<form action="../action/viewCheckout" method="get">			
					...
					<c:otherwise>
						...
						<div class="toolbar">
         	   		<h2 class="section" style="float:left; margin-top: 0;"><bean:write name="subheadApprovalRequired" /></h2>
	        	...
	        	<ul class="lightbox clearfix">
         		<logic:iterate name="assetBoxForm" property="listApprovalRequired" id="assetinlist" indexId="index">
         			...
         			<bean:define id="assetState" value="3"/>
         			<bean:define id="listTotal" name="assetBoxForm" property="noApprovalRequired"/>
         			<c:set var="collection" value="${imageDetailReturnName}: ${subheadApprovalRequired}"/>
					<%@include file="../public/inc_assetbox_asset.jsp"%>	 		
         		</logic:iterate>	
         	</ul>
         ...
         <c:if test="${approvalPendingSize > 0}">
			...
			<h2 class="section"><bean:write name="subheadPendingApproval" /></h2>
         	...
         	<bright:cmsWrite identifier="lightbox-pending-approval" filter="false" />
	        	...
	        	<ul class="lightbox clearfix">
         		...
         		<logic:iterate name="assetBoxForm" property="listApprovalPending" id="assetinlist" indexId="index">
         			<bean:define id="asset" name="assetinlist" property="asset" />				
         			<bean:define id="assetState" value="4"/>
         			<bean:define id="listTotal" name="assetBoxForm" property="noApprovalPending"/>
         			<c:set var="collection" value="${imageDetailReturnName}: ${subheadPendingApproval}"/>
					<%@include file="../public/inc_assetbox_asset.jsp"%>	 		
         		</logic:iterate>
			...
			<c:if test="${usePriceBands && purchaseRequiredWithoutPriceBandsSize > 0}">
				...
				<h2 class="section">
					<bean:write name="subheadPrevAdded" />
				</h2>
				...
				<ul class="lightbox clearfix">
					<logic:iterate name="assetBoxForm" property="listPurchaseRequiredWithoutPriceBands" id="assetinlist" indexId="index">
						<bean:define id="asset" name="assetinlist" property="asset" />				
						<bean:define id="assetState" value="-1"/>
         				<bean:define id="listTotal" value="-1"/>
         				<c:set var="collection" value="${imageDetailReturnName}: ${subheadPrevAdded}"/>
						<%@include file="../public/inc_assetbox_asset.jsp"%>	 		
					</logic:iterate>		
				</ul>
				<div class="hr"></div>
         ...
         <c:if test="${approvalRejectedSize > 0}">
				...
				<c:choose>
					...
					<h2 class="section">
		         		<bean:write name="subheadApprovalRejected" />
		         	</h2>
         	...
         	<ul class="lightbox clearfix">
         		<logic:iterate name="assetBoxForm" property="listApprovalRejected" id="assetinlist" indexId="index">
         			<bean:define id="asset" name="assetinlist" property="asset" />				
         			<bean:define id="assetState" value="5"/>
         			<bean:define id="listTotal" name="assetBoxForm" property="noApprovalRejected"/>
         			<c:set var="collection" value="${imageDetailReturnName}: ${subheadPurchaseRejected}"/>
					<%@include file="../public/inc_assetbox_asset.jsp"%>	 		
         		</logic:iterate>		
         	</ul>
				<div class="hr"></div>
         ...
         <c:if test="${viewOnlySize > 0}">
			...
			<h2 class="section">
         		<bean:write name="subheadViewOnly" />
         	</h2>
         	...
         	<ul class="lightbox clearfix">
         		<logic:iterate name="assetBoxForm" property="listViewOnly" id="assetinlist" indexId="index">
         			<bean:define id="asset" name="assetinlist" property="asset" />				
         			<bean:define id="assetState" value="6"/>
         			<bean:define id="listTotal" name="assetBoxForm" property="noViewOnly"/>
         			<c:set var="collection" value="${imageDetailReturnName}: ${subheadViewOnly}"/>
					<%@include file="../public/inc_assetbox_asset.jsp"%>	 		
         		</logic:iterate>		
         	</ul>
				<div class="hr"></div>				
