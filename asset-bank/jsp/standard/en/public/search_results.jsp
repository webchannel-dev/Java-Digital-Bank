...
<head>
	...
	<c:set var="isQuickSearch" value="${not empty param.quickSearch}" />
	<c:if test="${!isQuickSearch}">
	...
	<c:if test="${isQuickSearch}">
		<script type="text/javascript" charset="utf-8">
			...
			<c:if test="${autoCompleteQuickSearchEnabled}">
				$j(function() {
					var $searchBox = $j('#searchKeywordsInPage');
					if ($searchBox.length>0) {
						//initialise jquery autocompleter on search field:
						initJQAutocompleter($searchBox, "");
						$searchBox.focus();
					}
				});
...
<body id="resultsPage">
	...
	<c:choose>
		<c:when test="${savedSearchesEnabled && userprofile.isLoggedIn && !isQuickSearch}">
   ...
   <div class="head stripBelow tabsAbove">
		...
		<c:if test="${not empty fromUrl && !isQuickSearch}">
		...
		<c:if test="${isQuickSearch}">
			<form action="quickSearch" method="get" style="margin: 1em 0 1.5em 0">
				<input type="hidden" value="true" name="newSearch">
				<input type="hidden" value="true" name="quickSearch">
				<input type="text" name="keywords" id="searchKeywordsInPage" value="<c:out value="${sQuery}" />" class="big" />
				<logic:notEmpty name="quickSearchEntities">
	
		  			<logic:greaterThan name="numEntities" value="1">
			  			<select name="selectedEntities" class="big">
			  				...
			  				<logic:iterate name="quickSearchEntities" id="entity">
			  					<c:forEach items="${searchForm.selectedEntities}" var="selectedEntity">
			  						<c:choose>
				  						<c:when test="${selectedEntity ==  entity.id}">
				  							<c:set var="entityIsSelected" value="true" />  
				  						</c:when>
				  						<c:otherwise>
				  							<c:set var="entityIsSelected" value="false" />
				  						</c:otherwise>
				  					</c:choose>
			  					</c:forEach>
			  					<option <c:if test="${entityIsSelected}"> selected </c:if> value="<bean:write name="entity" property="id"/>"><bean:write name="entity" property="name" /></option>
			  				</logic:iterate>
	  			...
	  			<c:if test="${showCategoriesOnQuickSearch != 'none'}">
	  				<c:set var="browserCategories" value="${searchForm.descriptiveCategoryForm.topLevelCategories}" scope="request" />
					<c:set var="bigStyle" value="big" scope="request" />
					<jsp:include page="inc_quick_search_categories_list.jsp" />
				</c:if>
				
				<input type="submit" class="button big" value="<bright:cmsWrite identifier="button-search" />"/>
				&nbsp;&nbsp;&nbsp;<a href="viewLastSearch?refineSearch=1"><bright:cmsWrite identifier="link-advanced-search" /></a>
			</form>
		</c:if>	
		...
		<ul class="linkList noBottomMargin">
		<c:choose>
			<c:when test="${searchForm.searchResults.numResults gt 0}">
								...
								<c:when test="${searchForm.searchResults.numResults!=1}">
									<c:choose>
										...
										<c:otherwise>
											...
											<!-- <c:out value="${numberHits}" /> results found	 -->
										</c:otherwise>
									</c:choose>				
								</c:when>				
								<c:otherwise>
				...
				<c:if test="${searchPage != 'batchrelease' && searchPage != 'adminSearch'}">
					<c:if test="${!isQuickSearch}">
						<li class="group">
			...
			<c:otherwise>
				...
				<c:if test="${!isQuickSearch}">
					...
					<c:if test="${searchPage != 'adminSearch'}">
			  			<li>
			  				<a href="viewLastSearch?refineSearch=1" class="floatLeft"><bright:cmsWrite identifier="refine-search-results" filter="false" /></a>
			  			</li>
		  			</c:if>
		...
		<div class="foot">
			...
			<c:if test="${searchPage != 'batchrelease' && searchPage != 'adminSearch'}">
				<ul class="linkList">	
					...
					<li><a href="viewLastSearch" class="floatLeft"><bright:cmsWrite identifier="link-search-again" filter="false" /></a></li>
					
					<c:if test="${savedSearchesEnabled && userprofile.isLoggedIn}">
						<li><a href="viewSaveSearch" class="floatLeft"><bright:cmsWrite identifier="link-save-search" filter="false" /></a></li>
					</c:if>
	
					<li><a href="viewLastSearch?refineSearch=1" class="floatLeft"><bright:cmsWrite identifier="refine-search-results" filter="false" /></a></li>
				
					<c:if test="${empty searchForm.selectAssetUrl && (userprofile.isAdmin || (orgUnitAdminsCanExport && userprofile.isOrgUnitAdmin))}">
						<li><a href="exportFromSearch" class="floatLeft">Export all results...</a></li>
					</c:if>
				</ul>
			</c:if>
		</div>
