			...
			<tr>
				<td>
					...
					<c:if test="${userprofile.isAdmin && not empty image.createdByUser}"> 
						by <bean:write name="image" property="createdByUser.fullName" /> <c:if test="${not empty image.createdByUser.emailAddress}">(<bean:write name="image" property="createdByUser.emailAddress" />)</c:if>
					</c:if>
