					...
					<c:when test="${attributeValue.attribute.typeId == 10}">
						<c:set var="groupHeaderContent">
							...
							<div class="attributeGroupHeading" >
								<h3 <c:if test="${!attributeValue.attribute.highlight}">class="collapsed"</c:if>><bean:write name="attributeValue" property="attribute.label"/><span class="ellipsis">...</span></h3>
							</div>
					...
					<c:choose>		
						...
						<c:when test="${attributeValue.attribute.fieldName == 'assetId'}">
							<tr <c:if test='${attributeValue.attribute.highlight}'>class="highlight"</c:if>>
								<th>
									<bean:write name="attributeValue" property="attribute.label"/>: 
								</th>
								<td class="padded">
						...
						<%-- Version field --%>
						<c:when test="${attributeValue.attribute.fieldName == 'version' && (asset.versionNumber>1 || asset.currentVersionId>0 || batchReleasesEnabled)}">
							<tr <c:if test='${attributeValue.attribute.highlight}'>class="highlight"</c:if>>
								<th>
									<bean:write name="attributeValue" property="attribute.label"/>: 
								</th>
								<td class="padded">
									...
									<c:if test="${ asset.versionNumber>1}">
										...
										<c:if test="${ asset.currentVersionId<=0}">
											...
											<c:if test="${userprofile.isAdmin && (!batchReleasesEnabled || !asset.currentBatchReleaseReleased)}">
							...
							<tr <c:if test='${attributeValue.attribute.highlight}'>class="highlight"</c:if>>
								...
								<td class="padded">
									...
									<c:if test="${asset.typeId==2 && asset.numPages > 1}">
										<bean:write name="asset" property="numPages" /> <bright:cmsWrite identifier="snippet-layers" filter="false"/>;
									</c:if>
						...
						<tr <c:if test='${attributeValue.attribute.highlight}'>class="highlight"</c:if>>
							...
							<td class="padded">
								<logic:notEmpty name="asset" property="addedByUser">
									<bean:write name="asset" property="addedByUser.fullName" />&nbsp;
									<logic:notEmpty name="asset" property="addedByUser.emailAddress">
										(<a href="mailto:<bean:write name="asset" property="addedByUser.emailAddress"/>"/>
											<bean:write name="asset" property="addedByUser.emailAddress" />
										</a>)
									</logic:notEmpty>
								</logic:notEmpty>
							</td>
						...
						<tr <c:if test='${attributeValue.attribute.highlight}'>class="highlight"</c:if>>
							...
							<td class="padded">
								<logic:notEmpty name="asset" property="lastModifiedByUser">
									<bean:write name="asset" property="lastModifiedByUser.fullName" />&nbsp;
									<logic:notEmpty name="asset" property="lastModifiedByUser.emailAddress">
										(<a href="mailto:<bean:write name="asset" property="lastModifiedByUser.emailAddress"/>"/>
											<bean:write name="asset" property="lastModifiedByUser.emailAddress" />
										</a>)
									</logic:notEmpty>
								</logic:notEmpty>
							</td>
						...
						<tr <c:if test='${attributeValue.attribute.highlight}'>class="highlight"</c:if>>
							...
							<c:choose>
								...
								<c:otherwise>
									<td class="padded">
										<bright:writeMoney name="asset" property="price.displayAmount"  /><c:if test="${userprofile.maxDiscount > 0}"> (A discount of <bean:write name='userprofile' property='maxDiscount'/>% will be applied to this when ordered)</c:if>
									</td>
								</c:otherwise>
							</c:choose>
						</tr>
										...
										<logic:iterate name="asset" property="descriptiveCategories" id="category" indexId="index">
											...
											<logic:notEmpty name="category" property="ancestors">
												<logic:iterate name="category" property="ancestors" id="ancestor">
													...
													<bean:write name="ancestor" property="name" /></a>/
													</c:if>
												</logic:iterate>
											</logic:notEmpty>
											<c:set var="linkCategory" value="${category}" />
											<%@include file="inc_attribute_browse_link.jsp"%>
											<bean:write name="category" property="name" /></a><c:if test="${(index+1) < noOfCategories}">, </c:if>
										</logic:iterate>&nbsp;
									</logic:notEmpty>
								</td>
							</tr>
						...
						<c:when test="${attributeValue.attribute.fieldName == 'accessLevels'}">
							<c:if test="${!hideAccessLevels}">
								<c:choose>
									<c:when test="${userprofile.isAdmin || (!asset.currentBatchReleaseReleased && userprofile.canManageBatchReleases)}">
									...
									<td class="padded">
										...
										<logic:notEmpty name="catsIterator">
											...
											<logic:iterate name="catsIterator" id="category" indexId="index">
												...
												<c:set var="linkCategory" value="${category}" />
												<%@include file="inc_attribute_browse_link.jsp"%>
												<bean:write name="category" property="name" /></a>
												<%
...
<c:if test="${userprofile.isAdmin}">
	...
	<c:if test="${not empty groupHeaderContent}">
		...
		<%-- Is Featured field --%>						
		<c:if test="${ !hideFeatured && (asset.isImage || asset.isVideo) && asset.surrogateAssetId<=0 && iFeaturedImageWidth gt 0}">
			<tr>
				<th>
							...
							<logic:iterate name="assetForm" property="brandList" id="brand">
								...
								<logic:iterate name="assetForm" property="brandSelectedList" id="brandSelectedId">
									<c:if test="${brand.id == brandSelectedId}">
										...
										<bean:write name="brand" property="name" />
