					...
					<c:if test="${relationshipTypeId>0 && (userprofile.isAdmin == true || assetForm.userCanUpdateAsset == true)}">
						<c:if test="${!supportMultiLanguage}">			
							<logic:notEmpty name='relationshipDescriptionLabel'>
								<c:set var="viewEditRelatedAssetDescriptionsUrl">viewEditRelatedAssetDescriptions?id=<bean:write name='assetForm' property='asset.id'/>&<c:choose><c:when test='${relationshipTypeId==1}'>peerAssetIds</c:when><c:when test='${relationshipTypeId==2}'>childAssetIds</c:when></c:choose>=<bean:write name='relatedAsset' property='id' />&amp;edit=true</c:set>
								<bright:a disabled="${linkDisabled}" styleClass="view" href="${viewEditRelatedAssetDescriptionsUrl}">Edit <bean:write name='relationshipDescriptionLabel' /></bright:a>
							</logic:notEmpty>
						</c:if>
						<c:choose>
    						<c:when test="${relatedAsset.userDeletableRelationship && assetForm.userCanUpdateAsset}">
								<bright:cmsSet var="jsConfirmDeleteRelationship" identifier="js-confirm-delete-relationship"/>
								<bright:a disabled="${linkDisabled}" styleClass="view" href="deleteAssetRelationship?parentId=${assetForm.asset.id}&childId=${relatedAsset.id}&relationshipTypeId=${relationshipTypeId}#related" onclick="return confirm('${jsConfirmDeleteRelationship}');"><bright:cmsWrite identifier="link-remove-link" filter="false"/></bright:a>
							</c:when>
							<c:otherwise>
						...
						<c:if test="${relationshipTypeId==2 && !lastRelatedAsset}">
							<bright:a disabled="${linkDisabled}" styleClass="view" href="moveAssetRelationship?id=${assetForm.asset.id}&childId=${relatedAsset.id}&i=${index}">Move down list</bright:a>
						</c:if>
					</c:if>
				</p>
			</li>
