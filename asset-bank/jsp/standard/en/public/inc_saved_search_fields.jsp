...
<html:hidden name="savedSearchForm" property="savedSearch.searchCriteriaId"/>
...
<html:hidden name="savedSearchForm" property="savedSearch.id"/>
	...
	<c:if test="${savedSearchForm.savedSearch.searchCriteriaId > 0}">
...
function checkPersonalTypeFields() {
	...
	if ( !savedSearchType ) {
		// Editing, rather than creating anew - the search type is predefined and locked
		savedSearchType = ${savedSearchForm.savedSearch.type}
	}
	var personalSearch = 1;
	var recommendedSearch = 2;
	var featuredSearch = 3;
	
	if( savedSearchType == personalSearch || ! isAdmin ) 
	{
	...
	if( isAdmin )
	{
		if( savedSearchType == featuredSearch ) 
		{
			displayDescriptionFields();
		}
		else 
		{
...
function getDescriptionFields() {
	return document.getElementById( "descriptionFields" );
}

function displayDescriptionFields() {
	getDescriptionFields().style.display = '';
}

function hideDescriptionFields() {
	getDescriptionFields().style.display = 'none';
}
