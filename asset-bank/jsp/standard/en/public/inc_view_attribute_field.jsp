...
<c:if test="${!attributeValue.attribute.isDataLookupButton && !attributeValue.attribute.isGroupHeading && attributeValue.attribute.id != reqLastAttId  && (!hideEmptyAtts || (!isEmpty || (attributeValue.attribute.isHyperlink && not empty attributeValue.attribute.defaultValue)))}">
	<tr <c:if test='${attributeValue.attribute.highlight}'>class="highlight"</c:if>>
		<c:if test="${!reqHideLabels}">
			<th>
				...
				<bean:write name="attributeValue" property="attribute.label"/><c:if test="${!empty attributeValue.attribute.label && attributeValue.attribute.label != ' ' }">:</c:if>
			</th>
		</c:if>
...
<pre><bean:write name="attributeValue" property="value" filter="true"/></pre> <%-- Leave this against the left hand edge to avoid padding in the value --%>
							...
							<c:if test="${searchTokens}">
								<c:choose>	
									...
									<c:otherwise>
										...
										<bright:write name="attributeValue" property="value" formatCR="true" filter="false" tokenDelimiter="<%= String.valueOf(delimiter) %>" outputTokenWrapper="<%= \" <a href='search?attribute_\" + attributeId + \"=[enc-token]'>[token]</a>\"%>"/>
									</c:otherwise>
								</c:choose>
							</c:if>
						</c:otherwise>
				...
				<c:when test="${attributeValue.attribute.isKeywordPicker}">
					...
					<logic:notEmpty name="attributeValue" property="keywordCategories">
						...
						<c:if test="${!displayKeywordsByRootAncestor}">
							...
							<logic:iterate name="attributeValue" property="keywordCategories" id="category" indexId="index"><logic:notEmpty name="category" property="ancestors"><logic:iterate name="category" property="ancestors" id="ancestor"><a href="../action/search?attribute_<bean:write name='attributeValue' property='attribute.id'/>=<bright:write name='ancestor' property='name' encodeForUrl="true"/>"><bean:write name="ancestor" property="name" /></a><c:out value="${keywordAncestorDelimiter}"/></logic:iterate></logic:notEmpty><a href="../action/search?attribute_<bean:write name='attributeValue' property='attribute.id'/>=<bright:write name='category' property='name' encodeForUrl="true"/>"><bean:write name="category" property="name" /></a><c:if test="${(index+1) < noOfCategories}"><c:out value="${keywordDelimiter}"/> </c:if></logic:iterate>&nbsp;
						</c:if>
						<c:if test="${displayKeywordsByRootAncestor}">
							<logic:iterate name="attributeValue" property="keywordMap" id="entry" indexId="parentIndex">
								...
								<logic:iterate name="entry" property="value" id="cat" indexId="childIndex">
									<logic:notEmpty name="cat" property="ancestors"><logic:iterate name="cat" property="ancestors" id="ancestor" offset="1"><a href="../action/search?attribute_<bean:write name='attributeValue' property='attribute.id'/>=<bright:write name='ancestor' property='name' encodeForUrl="true"/>"><bean:write name="ancestor" property="name" /></a><c:out value="${keywordAncestorDelimiter}"/></logic:iterate></logic:notEmpty><a href="../action/search?attribute_<bean:write name='attributeValue' property='attribute.id'/>=<bright:write name='cat' property='name' encodeForUrl="true"/>"><bean:write name="cat" property="name" /></a><c:if test="${(childIndex+1) < noOfChildren}"><c:out value="${keywordDelimiter}"/></c:if>
								</logic:iterate>
							</logic:iterate>
						</c:if>
					</logic:notEmpty>
				...
				<c:when test="${attributeValue.attribute.isHyperlink}">
					...
					<c:if test="${isEmpty && not empty attributeValue.attribute.defaultValue}">
						<a href="<bean:write name="attributeValue" property="attribute.defaultValue" filter="false"/>" TITLE="<bean:write name="attributeValue" property="attribute.altText" />" target="_blank"><bean:write name="attributeValue" property="attribute.displayName" /></a><br/>
					</c:if>
							...
							<c:if test="${not empty pair.value || not empty pair.name}">
								...
								<c:choose>
									<c:when test="${empty attributeValue.attribute.tokenDelimiterRegex}">
										<c:if test="${not empty pair.value}"><a href="<bean:write name="linkUrl"/>" TITLE="<bean:write name="attributeValue" property="attribute.altText" />" target="_blank"></c:if><c:if test="${empty pair.name}"><c:if test="${not empty attributeValue.attribute.displayName}"><bean:write name="attributeValue" property="attribute.displayName" />: </c:if><bean:write name="pair" property="value"/></c:if><c:if test="${not empty pair.name}"><c:if test="${not empty pair.value}"><bean:write name="pair" property="value" /> - </c:if><bean:write name="pair" property="name" /></c:if><c:if test="${not empty pair.value}"></a></c:if>
									...
									<c:otherwise>
										<c:if test="${not empty pair.value}">
											<a href="../action/search?attribute_<c:out value="${attributeValue.attribute.id}"/>=%22<bright:write name="pair" property="value" encodeForUrl="true"/>%22"><bean:write name="pair" property="value" /></a>
											<c:if test="${not empty pair.name}">-</c:if>
										</c:if>              
										<bean:write name="pair" property="name"/>
										<c:if test="${not empty pair.value}">
											&nbsp;[<a href="<bean:write name="linkUrl"/>" TITLE="<bean:write name="attributeValue" property="attribute.altText" />" target="_blank"><bright:cmsWrite identifier="link-follow-link" filter="false"/></a>]
										</c:if>
									</c:otherwise>
								</c:choose>
								<br/>
				...
				<c:when test="${attributeValue.attribute.isFile}">
					...
					<a href="downloadAttributeFile?assetId=<bean:write name="assetForm" property="asset.id" />&attributeId=<bean:write name="attributeValue" property="attribute.id" />" 
					   title="<bean:write name="attributeValue" property="attribute.altText" />" target="_blank"><bean:write name="attributeValue" property="filename" /></a><br/>
