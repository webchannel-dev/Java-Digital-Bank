	...
	<c:if test="${downloadForm.downloadAsDocument}">    
      ...
      <p class="js-enabled-hide">
		...
		<form action="<bean:write name='actionUrl'/>" method="get" name="downloadForm" id="downloadForm" >
			...
			<c:choose>
				<c:when test="${!usageIsSelected}">
					<html:hidden name="downloadForm" property="repurpose"/>
				</c:when>
				
				<c:otherwise> <%--  Usage is selected --%>
					...
					<%-- Display checkbox options if: there are sizes or usage type is download original or in advanced form --%>
					<%-- Note: the action should ensure that there are either formats or can download, once a usage type chosen --%>
					<c:if test="${numUsageTypeFormats gt 0 || downloadForm.userCanDownloadOriginal || advanced}">
						
						<%@include file="inc_tscs_selection.jsp"%>
						
						<%--  Don't show options if the user needs high res approval, or is from cms --%>
						<c:if test="${!userprofile.isFromCms && !bRequiresHighResApproval}">
					...
					<c:choose>
						...
						<c:otherwise>
							...
							<%-- Download original option - if user has permission, and if advanced/not simple or usage type is download original	--%>
				   		...
				   		<%-- In the case of simple convert and simple tab and download original usage type and can download original and no sizes then do not show the convert --%>							
							<c:if test="${ !(simpleConvertOptionsForDownload && !advanced && downloadForm.selectedUsageType.usageType.downloadOriginal && downloadForm.userCanDownloadOriginal && numUsageTypeFormats==0) }">
								...
								<div class="dialogPanel" style="margin-bottom:0;">
										...
										<c:otherwise>
											...
											<c:choose>
												...
												<c:when test="${numUsageTypeFormats == 1 && downloadForm.usageTypeFormats[0].cropToFit && !(downloadForm.selectedUsageType.usageType.downloadOriginal && downloadForm.userCanDownloadOriginal)}">
													<label for="usagetypeformat" class="narrow"><bright:cmsWrite identifier="label-size" filter="false"/></label> <bean:write name='downloadForm' property='usageTypeFormats[0].description' /><br />
											...
											<%-- Determine whether to show the colorspace dropdown --%>
									...
									<br />
								</div><!-- end of dialog panel-->
							</c:if>
							
						</c:otherwise> <%--  End not approval --%>
					</c:choose>
					
				</c:otherwise> <%--  End usage is selected --%>
			</c:choose>
