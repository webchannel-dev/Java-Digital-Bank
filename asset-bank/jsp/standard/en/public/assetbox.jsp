			...
			<c:if test="${multipleLightboxes && userprofile.numAssetBoxes>1}">
					<c:choose>
						<c:when test="${assetBoxForm.publicPage!=1}">
							...
							<select name="currentAssetBoxId" onchange="this.form.submit();">
							<logic:iterate name="userprofile" property="assetBoxes" id="assetBox">
								<option value="<bean:write name='assetBox' property='id' filter='false'/>" <c:if test="${userprofile.assetBox.id==assetBox.id}">selected="selected"</c:if>><bean:write name='assetBox' property='name' filter='false'/> (<bean:write name='assetBox' property='assetBoxSize' filter='false'/>)</option>
							</logic:iterate>
							</select>
	...
	<%-- Other sections --%>
         ...
         <c:if test="${canDownloadSize + approvedSize > 0}">
			...
			<form action="../action/actionOnSelectedAssets" method="post" id="actionOnSelected">
				...
				<c:set scope="session" var="subheadReadyDownload"><bright:cmsWrite identifier="subhead-ready-download" /></c:set>
					...
					<div class="group first">
         			...
         			<div id="dropButtonSelected">
         				...
         				<div class="dropHolder">
         					<a href="#" class="button dropButton last" title="<bright:cmsWrite identifier="snippet-actions-on-selected-title-attr" filter="false"/>" onclick="return false;"><span></span><bright:cmsWrite identifier="button-lightbox-more-actions"/></a>
