			...
			<div class="newsPanel">
				...
				<logic:iterate name="newsItems" id="newsItem" indexId="newsItemIndex">
					<div class="newsContent">
						...
						<strong class="headline"><bean:write name="newsItem" property="name" /></strong>
						<p>
						<bean:write name="newsItem" property="content" filter="false"/>
						<c:if test="${newsItem.isTruncated}">
							<c:if test="${newsMaxCharacters > 0}">... <br /></c:if><a href="viewNewsItem?id=<bean:write name="newsItem" property="id" />"><bright:cmsWrite identifier="link-more" filter="false"/></a>
						</c:if>
						</p>
					</div>
				</logic:iterate>
