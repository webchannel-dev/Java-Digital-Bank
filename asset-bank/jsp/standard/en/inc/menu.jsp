		   ...
		   <c:if test="${(!extraContentAdminOnly || userprofile.isAdmin) && (!extraContentLoggedInOnly || userprofile.isLoggedIn)}">
				<c:if test="${not empty noOfExtraContentPages && noOfExtraContentPages > 0}">
					...
					<c:forEach begin="1" end="${noOfExtraContentPages}" var="index">
						...
						<li><a href="../action/viewContent?index=<c:out value='${index}'/>&showTitle=true" id="nav_content_${indexId}" <logic:equal name='section' value='<%= "extracontent" + indexId %>'>class="current"</logic:equal>><bright:cmsWrite identifier='<%="content-page-menu-"+indexId%>'/></a></li>
            ...
            <c:if test="${userprofile.canUploadAtAll}">
	            <li class="subholder"><a href="#" id="upload_nav_link" class="expanding"><bright:cmsWrite identifier="menu-upload" filter="false"/></a>
						<ul id="upload_nav">
							...
							<c:if test="${!batchReleasesEnabled}">
								<li class="last"><a href="../action/viewDefaultMyUploads" id="nav_unsubmitted" <logic:equal name='section' value='my-uploads'>class="current"</logic:equal>><bright:cmsWrite identifier="menu-my-uploads" filter="false"/></a></li>
							</c:if>
						</ul>
	            </li>   
			</c:if>
			
			<c:if test="${userprofile.canSubmitRequests || userprofile.canManageRequests || userprofile.isFulfiller}"><li><a href="../action/<c:choose><c:when test='${userprofile.canSubmitRequests}'>viewMyRequests</c:when><c:otherwise>viewManageRequests</c:otherwise></c:choose>" id="nav_requests" <logic:equal name='section' value='requests'>class="current"</logic:equal>><bright:cmsWrite identifier="menu-requests" filter="false"/></a></li></c:if>
					...
					<div class="lbPanel" id="lbPanel" style="margin-top:2em">
						...
						<div class="dropHolder singleButton">
							...
							<c:if test="${multipleLightboxes && userprofile.isLoggedIn}">
								...
								<div class="dropOptions <c:if test="${userprofile.numAssetBoxes>10}">scroll</c:if>">
									<ul>
										<logic:iterate name="userprofile" property="assetBoxes" id="assetBox">
											<li <c:if test="${userprofile.assetBox.id==assetBox.id}">class="current"</c:if>><a href="../action/switchAssetBox?currentAssetBoxId=<bean:write name='assetBox' property='id' />" class="lbOption"><bean:write name='assetBox' property='name' /> (<span class="count"><bean:write name='assetBox' property='assetBoxSize' /></span>)</a></li>
										</logic:iterate>
									</ul>
									<div class="add">
						...
						<c:if test="${multipleLightboxes && userprofile.numAssetBoxes>1}">
							...
							<form action="switchAssetBox" method="get" >
								<select style="margin-left: 8px; margin-top:0.6em; width:84px" name="currentAssetBoxId" id="selectLightbox" onchange="this.form.submit();">
									<logic:iterate name="userprofile" property="assetBoxes" id="assetBox">
										<option value="<bean:write name='assetBox' property='id'/>" <c:if test="${userprofile.assetBox.id==assetBox.id}">selected="selected"</c:if>><bean:write name='assetBox' property='name' /></option>
									</logic:iterate>
								</select>
									...
									<c:choose>
										<c:when test="${usePriceBands}">
											...
											<c:choose>
												...
												<c:otherwise>
													<strong><bright:writeMoney name="userprofile" property="totalWithDiscountPriceBands.displayAmount" /></strong>
												</c:otherwise>
											</c:choose>														
										</c:when>
										<c:otherwise>
											...
											<strong><bright:writeMoney name="userprofile" property="totalWithDiscount.displayAmount" /></strong>														
										</c:otherwise>
									</c:choose>
								</p>
							</c:if>
									...
									<logic:notEmpty name="filters">
										...
										<select name="filterId<bean:write name='group' property='id'/>" id="filterId<bean:write name='group' property='id'/>" onchange="document.getElementById('filterForm').submit();" >
											...
											<logic:iterate name="filters" id="currentFilter">
												...
												<option value="<bean:write name='currentFilter' property='id'/>" <c:if test='${selectedFilter}'>selected="selected"<c:set var="found" value="true"/></c:if>><bean:write name="currentFilter" property="name" /></option>
											</logic:iterate>
										</select>
