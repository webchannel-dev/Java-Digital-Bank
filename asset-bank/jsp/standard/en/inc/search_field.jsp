		...
		<c:when test="${attribute.static}">
			...
			<c:choose>		
				...
				<c:when test="${attribute.fieldName == 'assetId'}">		
					...
					<tr>
						<th>
							<label for="ids"><bean:write name="attribute" property="label" />:</label> 
						</th>
						<td>
				...
				<c:when test="${attribute.fieldName == 'orientation'}">		
					...
					<tr>
						<th>
							<label for="orientation"><bean:write name="attribute" property="label" />:</label> 
						</th>
						<td>
				...
				<c:when test="${ecommerce && attribute.fieldName == 'price'}">
					...
					<tr>
						<%-- Get the label --%>
						<th>
							<label for="price"><bean:write name="attribute" property="label" /> (<bean:write name="searchForm" property="priceLower.currencySymbol" filter="false" />):</label> 
						</th>
						<td>
				...
				<c:when test="${attribute.fieldName == 'sensitive'}">		
					...
					<tr>
						<th>
							<label for="sensitive"><bean:write name="attribute" property="label" /></label> 
						</th>
						<td>
					...
					<tbody class="attributeGroupHeading">
						<tr>
							<td colspan="3" style="padding-right:0px">
								<h3 <c:if test="${!attribute.highlight}">class="collapsed"</c:if> title="Show or hide these fields"><bean:write name="attribute" property="label" /><span class="ellipsis">...</span></h3>
			...
			<c:choose>
				...
				<c:when test="${attribute.isDropdownList}">
					...
					<select name="attribute_<bean:write name='attribute' property='id'/>" id="field<bean:write name='attribute' property='id'/>">
						...
						<logic:iterate name="attribute" property="listOptionValues" id="optionValue">
							<option <c:if test="${valueToRefine == optionValue.value}">selected</c:if> value="<bean:write name='optionValue' property='value' filter='false'/>" ><bean:write name='optionValue' property='value' filter='false'/></option>
						</logic:iterate>
				...
				<c:when test="${attribute.isKeywordPicker}">
					...
					<c:if test="${useKeywordChooser}">
						<script type="text/javascript">
							document.write('&nbsp;<a href="javascript:;" onclick="openKeywordChooser(true, <c:out value='${attribute.treeId}'/>, <c:out value='${attribute.id}'/>, 0<c:if test='${attribute.defaultKeywordFilter != null && attribute.defaultKeywordFilter != ""}'>, \'<bean:write name="attribute" property="defaultKeywordFilter" />\'</c:if>); return false;" onkeypress="openKeywordChooser(true, <c:out value='${attribute.treeId}'/>, <c:out value='${attribute.id}'/>, 0<c:if test='${attribute.defaultKeywordFilter != null && attribute.defaultKeywordFilter != ""}'>, \'<bean:write name="attribute" property="defaultKeywordFilter" />\'</c:if>); return false;" title="Popup keyword selector"><img src="../images/standard/icon/keyword_chooser.gif" class="help" alt="Choose Keywords" height="15" width="15" border="0" /><\/a>'); 
						</script>
					</c:if>
					<br />
					<c:if test="${attribute.autoComplete && attribute.isVisible && autoCompleteEnabled}">
				...
				<c:when test="${attribute.isSpatialArea}">
					...
					<div class="spatialAreaWrap">
						...
						<p class="noTopMargin areaPointToggle  tabHolder">
