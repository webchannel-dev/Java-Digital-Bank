...
<c:choose>
	<c:when test='${ numResults > 0 }'>
		...
		<div class="toolbar">
			<logic:present name="panel" property="header">
				<h2>
					<bean:write name="panel" property="header" /> 
		...
		<c:set var="categoryPair">
			<c:out value="${browseItemsForm.category.id}" />:<c:out value="${browseItemsForm.category.categoryTypeId}" />
		</c:set>
		<c:choose>
			<c:when test="${panel.view == 'list'}">
				...
				<c:choose>
					<c:when test="${panel.showExtraDescriptions}">
						<bright:refDataList componentName="DisplayAttributeManager" methodName="getDisplayAttributeLabelsIncExtraDescriptionsFromRefData"	id="displayAttributes" transactionManagerName="DBTransactionManager" passUserprofile="true" argumentValue="${5}" argument2Value="${categoryPair}"/>
					</c:when>
					<c:otherwise>
						<bright:refDataList componentName="DisplayAttributeManager" methodName="getDisplayAttributeLabelsFromRefData"	id="displayAttributes" transactionManagerName="DBTransactionManager" passUserprofile="true" argumentValue="${5}" argument2Value="${categoryPair}"/>
					</c:otherwise>
				</c:choose>
			...
			<c:when test="${panel.view == 'minimal'}">
				<c:choose>
					<c:when test="${panel.showExtraDescriptions}">
						<bright:refDataList componentName="DisplayAttributeManager" methodName="getDisplayAttributeLabelsIncExtraDescriptionsFromRefData"	id="displayAttributes" transactionManagerName="DBTransactionManager" passUserprofile="true" argumentValue="${11}" argument2Value="${categoryPair}"/>
					</c:when>
					<c:otherwise>
						<bright:refDataList componentName="DisplayAttributeManager" methodName="getDisplayAttributeLabelsFromRefData"	id="displayAttributes" transactionManagerName="DBTransactionManager" passUserprofile="true" argumentValue="${11}" argument2Value="${categoryPair}"/>
					</c:otherwise>
				</c:choose>
	...
	<c:otherwise>
		...
		<c:if test="${browseItemsForm.hasCategory}">
			<div class="toolbar">
				<c:if test="${not empty panel.header}">
					<h2 style="margin-top: 1em;" ><bean:write name="panel" property="header" /></h2>
					<ul class="toolbarActions">
