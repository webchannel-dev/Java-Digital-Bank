		...
		<c:choose>
			<c:when test="${ecommerce && usePriceBands}">
				...
				<logic:notEqual name="item" property="inAssetBox" value="true">
					<a id="ajaxAddToLightBox<c:out value='${item.id}'/>" class="add" style="display:block;" href="../action/addToAssetBox?<c:out value='id=${item.id}&${forwardParams}' />">					
				...
				<logic:equal name="item" property="inAssetBox" value="true">
					<a class="in" style="display:block;"><c:if test="${userprofile.numAssetBoxes>1}"><bright:cmsWrite identifier="link-in-lightbox" filter="false" /></c:if><c:if test="${userprofile.numAssetBoxes<=1}"><bright:cmsWrite identifier="link-in-my-lightbox" filter="false" /></c:if></a>														
				</logic:equal>
