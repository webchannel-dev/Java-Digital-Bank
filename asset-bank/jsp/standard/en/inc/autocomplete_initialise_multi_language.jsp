...
<%@ taglib uri="/WEB-INF/bright-tag.tld" prefix="bright" %>
	...
	<c:if test="${attribute.autoComplete && attribute.isVisible}">
		if ($j('#field<bean:write name='attribute' property='id'/>_<bean:write name="translation" property="language.id"/>').length>0) {
			...
			initJQAutocompleter($j("#field<bean:write name='attribute' property='id'/>_<bean:write name="translation" property="language.id"/>"), delimiters,"<bean:write name='attribute' property='id'/>","<c:out value="${searchOrEdit}"/>","<bean:write name="translation" property="language.code"/>");          
		}
	</c:if>
