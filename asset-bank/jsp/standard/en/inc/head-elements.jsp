...
<%-- If using as a jsp:include this file requires userprofile & section (and optionally dirValue) --%>
...
<%-- rescope the variables so this can be used via a jsp:include if necessary --%>
<c:set var="autoCompleteQuickSearchEnabled" value="${autoCompleteQuickSearchEnabled}" scope="request" />
<c:set var="categoryExplorerSetting" value="${categoryExplorerSetting}" scope="request" />
<c:set var="useBrands" value="${useBrands}" scope="request" />
<c:set var="daysBeforeMonths" value="${daysBeforeMonths}" scope="request" />
<c:set var="jqueryDateFormat" value="${jqueryDateFormat}" scope="request" />
<c:set var="jqueryYearMonthFormat" value="${jqueryYearMonthFormat}" scope="request" />
   ...
   <link rel="stylesheet" type="text/css" href="../css/standard/ie-8.css" />
	...
	<c:choose>
		<c:when test="${useBrands && userprofile.brand.id gt 0}">
			<c:set var="cssPath" value="../css/standard/${userprofile.brand.cssFile}" scope="request" />
