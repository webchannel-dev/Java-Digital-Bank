...
<%@ taglib uri="/WEB-INF/bright-tag.tld" prefix="bright" %>
	...
	<%-- set the detail page return url --%>
	<c:set var="queryString" value="?categoryId=${browseItemsForm.category.id}&categoryTypeId=${browseItemsForm.category.categoryTypeId}&allCats=${allCats}&sortAttributeId=${browseItemsForm.sortAttributeId}&sortDescending=${browseItemsForm.sortDescending}" />
	<c:choose>
		<c:when test="${explorer}">
			<c:set var="linkUrl" value="browseExplorerItems${queryString}"/>
		</c:when>
		<c:otherwise>
			<c:set var="linkUrl" value="browseItems${queryString}"/>
			<c:set var="imageDetailReturnUrl" scope="session" value="/action/${linkUrl}&page=${userprofile.selectedPage}&pageSize=${userprofile.selectedPageSize}"/>
	...
	<c:if test="${categoryExtensionAssetsEnabled && browseItemsForm.extensionAsset != null}">
		...
		<div id="extensionasset">
			...
			<bean:define id="hideTitle" value="false" />
			<logic:notEmpty name='descriptions'><bean:define id="hideTitle" value="true" /></logic:notEmpty>
