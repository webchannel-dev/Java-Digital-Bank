...
<c:choose>
	<c:when test="${ctrlHideAndSelectSingleCategory == 1 && !showSingleAccessLevel && ( (userprofile.isAdmin && numCats == 2) || (!userprofile.isAdmin && numCats==1))}">
		<%-- For search do not include the control at all, otherwise select it --%>
		<c:if test="${!bIsSearch}">
	...
	<c:otherwise>
		...
		function doEmptyCategoryFilter_<c:out value="${ctrlTree}"/>(a_listBox, iChangedListNumber, tree)
		{
			...
			if (subListControl)
			{
				if(subListControl.options.length > 0)
				{
					...
					{
						if(isCategoryEmpty_<c:out value="${ctrlTree}"/>(subListControl.options[i].value))
						{
		...
		<logic:equal name="userprofile" property="isAdmin" value="false">
			function isCategoryEmpty_<c:out value="${ctrlTree}"/>(i)
			{
		...
		document.write('				<select class="category" id="cat<bean:write name='ctrlTree' />-1" name="cat<bean:write name='ctrlTree' />-1" onchange="changedCategory(this, -1, \'<bean:write name='ctrlTree' />\');doEmptyCategoryFilter_<c:out value="${ctrlTree}"/>(this, -1, \'<bean:write name='ctrlTree' />\');">');
		...
		document.write('					<select class="category" id="cat<bean:write name='ctrlTree' /><bean:write name='subIndex'/>" onchange="changedCategory(this, <bean:write name='subIndex'/>, \'<bean:write name='ctrlTree' />\');doEmptyCategoryFilter_<c:out value="${ctrlTree}"/>(this, <bean:write name='subIndex'/>, \'<bean:write name='ctrlTree' />\');">     ');
			...
			<c:if test="${item.depth != 0 && (numCats lt maxNumCatsForNonJS || item.depth < 2)}">
				<c:choose>
					<c:when test="${item.depth == 1}">
						<c:if test="${checkboxIndex <= 0}">
							<c:set var="checkboxIndex" value="${0 - checkboxIndex}"/>
						</c:if>
				...
				<c:if test="${(checkboxIndex == checkboxColLength+1) || (checkboxIndex == (checkboxColLength*2)+1)}">
