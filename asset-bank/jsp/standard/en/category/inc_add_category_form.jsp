<h3>Add new sub-<bean:write name='catName' /> to <bean:write name='currentCategoryName' />:</h3>
...
<form action="<bean:write name='addAction' />" method="post" class="floated">	
	...
	<input type="hidden" name="catExtensionReturnLocation" value="<c:out value='${catExtensionReturnLocation}' />" />
	
	<c:if test="${catExtensionAssetsEnabled}">
		<c:choose>
			<c:when test="${forceExtensionAsset}">
				<html:hidden name="categoryAdminForm" property="extendedCategory" value="true" />
			</c:when>
			<c:otherwise>
	...
	<c:choose>
		<c:when test="${not empty paneller}">
			<label>Display assets in panels?</label>
			<bright:applicationSetting id="defaultToPanels" settingName="default-category-browsing-to-panels" />
			<input type="checkbox" class="checkbox" name="newCategory.browseAsPanels" value="true" <c:if test='${defaultToPanels}'>checked</c:if>/><br />
		</c:when>
		<c:otherwise>
			<html:hidden name="categoryAdminForm" property="newCategory.browseAsPanels" value="false" />
		</c:otherwise>
	</c:choose>
		...
		<logic:notEmpty name="categoryAdminForm" property="newCategory.translations">
			<logic:iterate name="categoryAdminForm" property="newCategory.translations" id="translation" indexId="tIndex">
				<logic:greaterThan name="translation" property="language.id" value="0">
							...
							<input type="text" class="text" name="newCategory.translations[<bean:write name='tIndex'/>].name" maxlength="150" size="32" id="newCatName<bean:write name='tIndex'/>" value="<bean:write name="translation" property="name" />"/><br />
