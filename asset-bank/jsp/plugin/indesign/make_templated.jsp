	...
	<script type="text/javascript">
		$j(document).ready(function() {
			...
			var handleEntityChange = function() {
				...
				showWarningIfEntityChanging(selectedAssetEntityId);
				restrictTemplatesByEntity(selectedAssetEntityId);
			}

			var restrictTemplatesByEntity = function(selectedAssetEntityId) {
			...
			var showWarningIfEntityChanging = function (newEntityId) {
				var entityChangeWarning = $j("#entityChangeWarning");
				if (newEntityId == <c:out value="${currentEntityId}"/>)
				{
					entityChangeWarning.addClass("hidden");
				}
				else
				{
					entityChangeWarning.removeClass("hidden");
				}
			};

			handleEntityChange();

			$j("#assetEntityId").change(handleEntityChange);
		});
	</script>
</head>
...
<body>
	...
	<div class="warning">
		<bright:cmsWrite identifier="snippet-indd-make-templated-warning"/>
	</div>

	<bright:cmsWrite identifier="intro-indesign-make-templated" filter="false"/>
	...
	<html:form action="inDesignMakeTemplated" method="POST" styleClass="floated">
		...
		<span id="entityChangeWarning" class="note hidden"><bright:cmsWrite identifier="snippet-change-entity-warning"/></span>
		<br />
