...
<head>
	...
	<title><bright:cmsWrite identifier="company-name" filter="false"/> | Edit Marketing Group</title>
...
<body id="adminPage">
	...
	<html:form action="editMarketingGroup" method="post">
		<html:hidden name="marketingGroupForm" property="group.id"/>
		<table cellspacing="0" class="form" summary="List of marketing groups">		
					...
					<logic:greaterThan name="translation" property="language.id" value="0">
						<tr>
							...
							<td>
								...
								<input type="text" class="text" name="group.translations[<bean:write name='tIndex'/>].name" maxlength="40" id="groupName<bean:write name='tIndex'/>" value="<bean:write name="translation" property="name" filter="false"/>"/>
							</td>
						</tr>
					</logic:greaterThan>
				</logic:iterate>
			...
			<tr>
				<th style="vertical-align:top; padding-top: 3px;">Notes<br/><span class="disabled" style="font-weight:normal;">(not visible to users)</span></th>
		...
		<c:if test="${supportMultiLanguage}">
			<p>Note, if a translated name is not provided for a particular language, the marketing group will be hidden from users viewing the application in that language.</p>
		</c:if>
