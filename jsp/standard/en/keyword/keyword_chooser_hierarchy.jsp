...
<bean:parameter id="filter" name="filter" value="a"/>
	...
	<div id="tabContent">
		...
		<logic:equal name="categoryAdminForm" property="subCategoryListIsEmpty" value="false">
			<table cellspacing="0" class="admin list" summary="Category list" style="width:100%">													
				<tr>
					...
					<tr>
						...
						<td style="padding-right:0;">
							<c:if test="${keywordAnscestorsSelectable || keyword.numChildCategories==0}">
								<a href="#" name="addbutton" onclick="addKeyword('<bean:write name='attributeId' />', '<bean:write name='keyword' property='nameWithEscapedQuotes' filter='false' />', '<bean:write name='keywordDelimiter' />'); return false;" id="<bean:write name='keyword' property='name' />">[<bright:cmsWrite identifier="link-add" filter="false"/>]</a>&nbsp;
							</c:if>
							<c:if test="${keyword.numChildCategories>0}">
