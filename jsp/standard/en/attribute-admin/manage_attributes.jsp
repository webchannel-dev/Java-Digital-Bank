...
<body id="adminPage">
	...
	<div id="tabContent">
		...
		<bright:refDataList componentName="AttributeManager" methodName="getVisibleAttributePositionList" id="attPositionList"/>
		...
		<table cellspacing="0" class="admin" summary="List of Attributes">
			...
			<c:if test="${!(attribute.fieldName=='price' && ecommerce!='true') && !(attribute.fieldName=='version' && canCreateAssetVersions!='true') && !((attribute.id==attributeConstants.k_lAttributeId_SensitivityNotes || attribute.fieldName=='sensitive') && !showSensitivityFields) && !(attribute.fieldName=='rating' && rating!='true') && !(attribute.fieldName=='agreements' && !agreementsEnabled) && !(attribute.fieldName=='audit' && !auditLoggingEnabled)}">	
				<tr>
					<td>
						<c:choose>
							<c:when test="${attribute.isGroupHeading}">
								<span style="text-decoration: underline"><bean:write name="attribute" property="label"/></span>
							</c:when>
							<c:otherwise>
					...
					<td class="action">
						[<c:if test="${attribute.sequence > 1}"><a href="moveAttribute?up=true&index=<bean:write name='index'/>"></c:if>up<c:if test="${attribute.sequence > 1}"></a></c:if>]
					</td>
					<td class="action">
						[<a href="moveAttribute?;up=false&index=<bean:write name='index'/>">down</a>]
					</td>
					<td class="action">
					...
					<td class="action">
						<c:choose><c:when test="${attribute.typeId!=attributeConstants.k_lAttributeType_GroupHeading && attribute.fieldName!='categories' && attribute.fieldName!='accessLevels'}">[<a href="hideAttribute?id=<c:out value='${attribute.id}' />">hide</a>]</c:when><c:otherwise>-</c:otherwise></c:choose>
					</td>
					<td class="action">
						<c:choose>
							<%-- Don't allow static attributes or the "special" sensitivity notes attribute to be deleted --%>
	...
	<p><a href="viewHiddenAttributes">View hidden attributes &raquo;</a></p>
