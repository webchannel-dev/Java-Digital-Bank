	...
	<html:form action="saveStaticAttribute" method="post">
		...
		<table cellspacing="0" class="form" summary="Form for attribute details">
			...
			<tr>
				<th>
					<c:choose>
						<c:when test="${attributeForm.attribute.fieldName=='description' || attributeForm.attribute.fieldName=='file' || attributeForm.attribute.fieldName=='keywords' || attributeForm.attribute.fieldName=='embeddedData' || attributeForm.attribute.fieldName=='lastModifiedBy' || attributeForm.attribute.fieldName=='usage'}">
							Show on search:
						</c:when>
						<c:otherwise>
							<label for="searchField">Show on search:</label>
						</c:otherwise>
					</c:choose>
				...
				<td>
					<c:choose>
						...
						<c:otherwise>
							<html:checkbox styleClass="checkbox" name="attributeForm" property="attribute.searchField" styleId="searchField"/>
						</c:otherwise>
					</c:choose>
				</td>
			</tr>
			...
			<tr>
			<c:if test="${attributeForm.attribute.fieldName=='assetId'}">
			<tr>
				<th style="width:120px;">
