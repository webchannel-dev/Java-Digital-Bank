...
<body id="adminPage">
	...
	<logic:empty name="assetReportForm" property="details">
		<p>There are no potentially duplicate assets in the system.</p>
		<p>If you are expecting to see duplicate assets shown then it may be that your naming attribute is not properly set. Please visit the <a href="viewManageDisplayAttributes">Display Attributes page</a> select a name attribute and reindex the images before returning to this report.</p>
	</logic:empty>
	<logic:notEmpty name="assetReportForm" property="details">
		<p>This report compares file sizes on assets in <bright:cmsWrite identifier="app-name" filter="false"/> to determine potential duplicates. It includes a filename comparison for images under <bean:write name="duplicate-asset-size-threshold"/> bytes in size.</p>
		...
		<table cellspacing="0" class="report" summary="Duplicate Asset Report">		
			...
			<logic:iterate name="assetReportForm" property="details" id="group">
				...
				<tr>
					<td style="padding:5px 5px 5px 0;"><a href="viewAsset?id=<bean:write name='asset' property='id'/>"><img src="../servlet/display?file=<bean:write name='asset' property='thumbnailImageFile.path'/>" alt="<c:out value='${asset.searchName}'/>" style="border: 1px solid #333"/></a></td>
					<td>
					...
					<td>
						<bean:write name="asset" property="dateAdded" format="dd/MM/yyyy"/>
					</td>
					<td>
					...
					<td>
						<input type="hidden" name="id<bean:write name='asset' property='id'/>" value="<bean:write name='asset' property='id'/>"/>
						<input type="checkbox" id="delete_id<bean:write name='asset' property='id'/>" name="delete_id<bean:write name='asset' property='id'/>" value="<bean:write name='asset' property='id'/>" onclick="setChoice(this,<bean:write name='asset' property='id'/>);"/>
					</td>
					<td>
