...
<c:set scope="session" var="editAssetReturnAction" value="../action/viewAsset?id=${assetForm.asset.id}"/>
...
<h1>
	...
	<c:if test="${not empty assetForm.asset.entity.name}">
		<bean:write name="assetForm" property="asset.entity.name" filter="false"/> <bright:cmsWrite identifier="snippet-details" case="mixed" filter="false" />
	</c:if>
</h1> 