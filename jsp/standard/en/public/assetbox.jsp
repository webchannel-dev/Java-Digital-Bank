			      	...
			      	<logic:equal name="showRequestOnCd" value="true">
				   	...
				   	<div class="group">	
					   	<html:select name="assetBoxForm" property="actionOnSelectedAssets">
						   	...
						   	<c:if test="${showBulkUpdate && userprofile.canUpdateAssets}">
							   	<html:option value="5"><bright:cmsWrite identifier="heading-bulk-update" filter="false"/></html:option>
						   	</c:if>
						   	<c:if test="${userprofile.canUpdateAssets && getRelatedAssets && !assetEntitiesEnabled && userprofile.assetBox.numAssets>1}">
							   	<html:option value="6"><bright:cmsWrite identifier='button-link-assets' filter='false'/></html:option>
						   	</c:if>

					   	</html:select>	
					
							<%-- The if to decide whether to show the 'update' actions or not - BB - What is this for?--%>
							<c:if test="${(showBulkUpdate || (getRelatedAssets && !assetEntitiesEnabled && userprofile.assetBox.numAssets>1)) && userprofile.canUpdateAssets}">
							</c:if>
					...
					<div class="group first">
         			...
         			<div id="dropButtonSelected">
         				...
         				<div class="dropHolder">
         					<a href="#" class="linkButton dropButton last" title="More actions on selected items" onclick="return false;"><bright:cmsWrite identifier="button-lightbox-more-actions"/></a>
							...
							<ul class="dropOptions">
								...
								<c:if test="${enableSlideshow && userprofile.assetBox.containsImages && slideshowRepurposingEnabled && userprofile.isAdmin}">
									<li><a href="#" onclick="submitLbAction(9)"><bright:cmsWrite identifier="snippet-create-custom-slideshow" filter="false"/></a></li>
								</c:if>
							</ul>
