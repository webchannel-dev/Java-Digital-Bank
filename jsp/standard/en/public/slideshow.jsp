	   	...
	   	<div id="controlPanel">
		   	<div class="header">
					...
					<c:if test="${enableSlideshowRepurposing && userprofile.isAdmin}">
						<a href="viewRepurposeSlideshow?type=<bean:write name='type'/>&page=<bean:write name='pageNumber'/>&pageSize=<bean:write name='pageSize'/>&captionIds=<bean:write name='captionIds'/>&displayTime=<bean:write name='controlPanelForm' property='displayTime'/>&fullList=<c:out value='${fullList}'/>&labels=<bean:write name='controlPanelForm' property='showAttributeLabels'/>" class="js-enabled-show embed">Create embeddable version</a>
					</c:if>
				...
				<form action="<bean:write name='action'/>" >
					...
					<fieldset>
			   		<h3>Slideshow settings:</h3>
						...
						<label for="refreshrate"><bright:cmsWrite identifier="snippet-display-time" filter="false"/> <html:text name="controlPanelForm" property="displayTime" size="4" style="width:2em" styleId="maxwidth" /></label><br/><label for="labels"><bright:cmsWrite identifier="snippet-show-attribute-labels" filter="false"/> <html:checkbox name="controlPanelForm" property="showAttributeLabels" styleId="labels" value="true" /></label><br/>
