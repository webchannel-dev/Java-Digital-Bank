...
<body id="detailsPage" <c:if test="${cacheLargeImages && largeImageToggle != 'off'}">class="largeViewEnabled <c:if test="${userprofile.largeImagesOnView}">largeViewOn</c:if>"</c:if>>
	...
	<div class="clearfix">
		...
		<c:choose>
			<c:when test="${cacheLargeImages && largeImageToggle != 'off' && userprofile.largeImagesOnView && !ecommerce}">
				<%@include file="../public/inc_view_image_asset_actions.jsp"%>
