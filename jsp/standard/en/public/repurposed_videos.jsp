			...
			<tr>
				<td>
					...
					<c:if test="${userprofile.isAdmin && not empty video.createdByUser}"> 
						by <bean:write name="video" property="createdByUser.fullName" filter="false"/> <c:if test="${not empty video.createdByUser.emailAddress}">(<bean:write name="video" property="createdByUser.emailAddress" filter="false"/>)</c:if>
					</c:if>
