					...
					<c:when test="${attributeValue.attribute.typeId == 10}">
						<c:set var="groupHeaderContent">
							...
							<div id="attributeGroupHeading">
								<a href="javascript:showHide('attributeGroup<c:out value="${attributeValue.attribute.id}"/>','headerIcon<c:out value="${attributeValue.attribute.id}"/>');">
									<h3><bean:write name="attributeValue" property="attribute.label" filter="false"/></h3><img border="0" id="headerIcon<c:out value="${attributeValue.attribute.id}"/>" align="right" style="position: relative; top: -11px;" src="../images/standard/icon/<c:if test="${attributeValue.attribute.highlight}">attr_collapse</c:if><c:if test="${!attributeValue.attribute.highlight}">attr_expand</c:if>.gif">
								</a>
							</div>
							<div class="attributeGroupPanel" id="attributeGroup<c:out value="${attributeValue.attribute.id}"/>" style="display:<c:if test="${attributeValue.attribute.highlight}">block</c:if><c:if test="${!attributeValue.attribute.highlight}">none</c:if>;">
					...
					<c:choose>		
						...
						<c:when test="${attributeValue.attribute.fieldName == 'agreements'}">
							...
							<c:if test="${enableAgreements}">
								<c:if test="${(asset.agreementTypeId == 2 && asset.agreement.id>0) || ((asset.agreement.id>0 || asset.previousAgreementsCount>0) && userprofile.isAdmin)}">
									<tr <c:if test='${attributeValue.attribute.highlight}'>class="highlight"</c:if>>
										<th>
