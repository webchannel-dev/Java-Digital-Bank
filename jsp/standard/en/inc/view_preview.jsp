...
<c:choose>
	<c:when test="${showFlashVideoOnViewDetails && not empty assetForm.asset.encryptedEmbeddedPreviewClipLocation}">
		...
		<c:set var="splashImage" value="../servlet/display.jpg?file=${asset.previewImageFile.path}"/>
		...
		<c:choose>
			...
			<c:otherwise>
				<c:choose>
					...
					<c:when test="${not empty asset.previewImageFile.path}">
						<c:set var="thumbSrc" value="../servlet/display?file=${asset.previewImageFile.path}"/>
					</c:when>
					<c:when test="${empty asset.previewImageFile.path && not empty assetForm}">
