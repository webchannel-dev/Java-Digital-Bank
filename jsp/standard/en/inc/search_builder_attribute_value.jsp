...
<c:choose>
	...
	<c:when test="${attribute.isDatepicker || attribute.isDateTime}">
		...
		<input type="text" size="17" maxlength="20" class="small" name="clauses[<c:out value="${index}"/>].value" value="<bean:write name="attributeForm" property="value"/>">(<c:out value="${dateFormatHelpString}" />) &nbsp;<a href="#" title="Date selector popup"><img class="icon" src="../images/standard/icon/calendar.gif" alt="Calendar" onclick="openDatePickerSupportUS(document.getElementsByName('clauses[<c:out value="${index}"/>].value')[0], <bean:write name='daysBeforeMonths' />)" width="16" height="15"></a> &nbsp;
	</c:when>
	...
	<c:when test="${attribute.isKeywordPicker}">
		...
		<c:if test="${useKeywordChooser}">
			&nbsp;<a href="javascript:;" onclick="openKeywordChooser(true, <c:out value='${attribute.treeId}'/>, <c:out value='${attribute.id}'/>, <c:out value='${index}'/><c:if test='${attribute.defaultKeywordFilter != null && attribute.defaultKeywordFilter != ""}'>, '<bean:write name="attribute" property="defaultKeywordFilter" filter="false"/>'</c:if>); return false;" onkeypress="openKeywordChooser(true, <c:out value='${attribute.treeId}'/>, <c:out value='${attribute.id}'/>, <c:out value='${index}'/><c:if test='${attribute.defaultKeywordFilter != null && attribute.defaultKeywordFilter != ""}'>, '<bean:write name="attribute" property="defaultKeywordFilter" filter="false"/>'</c:if>); return false;" title="Popup keyword selector"><img src="../images/standard/icon/keyword_chooser.gif" class="help" alt="Choose Keywords" height="15" width="15" border="0" /></a> 
		</c:if>
	</c:when>	
	<c:when test="${attribute.isNumeric}">
		...
		<input type="text" size="17" maxlength="<c:if test="${attribute.isNumeric}">38</c:if>" id="field<bean:write name='attribute' property='id'/>_<c:out value='${index}'/>" class="small" name="clauses[<c:out value="${index}"/>].value" value="<bean:write name="attributeForm" property="value"/>">
	</c:when>
	<c:otherwise>
		...
		<c:if test="${(attribute.id==0) && autoCompleteEnabled}">
			<div id="autocomplete_field<bean:write name='attribute' property='id'/>_<c:out value='${index}'/>" class="autocomplete"></div>
			<script type="text/javascript">
				initAutocompleter("field<bean:write name='attribute' property='id'/>_<c:out value='${index}'/>", "autocomplete_field<bean:write name='attribute' property='id'/>_<c:out value='${index}'/>","","","search"); 
			</script>
