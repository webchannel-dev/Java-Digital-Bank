...
<c:if test="${searchForm.selectedFilter.id > 0}">
	<logic:notEmpty name="searchBuilderForm" property="filters">
		<logic:notEmpty name="searchBuilderForm" property="selectedFilter.hiddenSearchAttributeValues">
			<logic:iterate name="searchBuilderForm" property="selectedFilter.hiddenSearchAttributeValues" id="attVal">
				<c:choose>
	...
	<logic:iterate name="searchBuilderForm" property="clauses" id="clause" indexId="index">
		<c:choose>
			<c:when test="${clause.isHidden}">
				...
				<input type="hidden" name="clauses[<c:out value='${index}'/>].attributeId" value="<c:out value='${clause.attributeId}'/>"/>
		...
		<li id="clause<c:out value="${index}"/>" <c:if test="${!clause.visible}">style="display: none;"</c:if>>
			...
			<html:select name="searchBuilderForm" property="clauses[${index}].attributeId" styleId="attributeId${index}" onchange="doAttributeSelection(this,${index});">
				<c:choose>
					<c:when test="${not empty attributes}">
						...
						<optgroup label="-----------------------------------------">
							<html:optionsCollection name="attributes" value="id" label="label"/>
						</optgroup>
					</c:when>
					<c:otherwise>
						<html:option value="0"><bright:cmsWrite identifier="snippet-general-keywords" filter="false"/></html:option>
...
<input type="submit" class="button flush floated" id="submitButton" onClick="removeHiddenClauses();" value="<bright:cmsWrite identifier="button-search" filter="false" />" />
