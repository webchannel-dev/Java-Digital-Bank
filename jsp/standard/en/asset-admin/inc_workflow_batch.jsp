		...
		<ul class="radioList">
			...
			<logic:iterate name="batchAssetForm" property="transitions" id="transition" indexId="index4">	
				<li class="clearfix">
					...
					<input  type="radio" class="radio" id="radio_<c:out value='${ais.assetId}' />_<c:out value='${transition.transitionNumber}' />" name="transition" value="<c:out value="${transition.transitionNumber}" />" onclick=showMessages(); <c:if test='${batchAssetForm.selectedTransition==transition.transitionNumber}'>checked="checked"</c:if>/>	
