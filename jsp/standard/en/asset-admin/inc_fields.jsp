...
<un:useConstants var="attributeConstants" className="com.bright.assetbank.attribute.constant.AttributeConstants" />
	...
	<table class="form" cellspacing="0" cellpadding="0" border="0">
		...
		<c:if test="${not empty assetForm.asset.previewImageFile && assetForm.asset.surrogateAssetId<=0}">
							...
							<c:choose>
								<c:when test="${attribute.isVisible}">
									<tr>
										...
										<c:if test="${bIsBulkUpdate}">
											<td>
												<select name="update_agreements" id="update_approved" style="width:auto;">
													<option value="skip">- <bright:cmsWrite identifier="snippet-skip" filter="false" /> -</option>
													<option value="replace"><bright:cmsWrite identifier="snippet-replace" filter="false" /></option>
												</select>	
											</td>
							...
							<c:when test="${attribute.typeId==5}">
								<logic:iterate name="attribute" property="listOptionValues" id="optionValue">
									<c:if test="${optionValue.isSelected}">
										<input type="hidden"  name="attribute_<bean:write name='attribute' property='id'/>_<bean:write name='optionValue' property='id'/>" value="<bean:write name='optionValue' property='id'/>">								
									</c:if>														
								</logic:iterate>
							</c:when>
							<%-- Dropdown list --%>
	...
	<c:set scope="request" var="ctrlIncludedSubtitle" value="Categories to be included:" />
