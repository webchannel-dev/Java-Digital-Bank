	...
	<html:form action="doSubscription" method="post">
					...
					<c:if test="${subscriptionForm.upgradePricesAvailable}">
						<td>
							<c:if test="${model.bestPrice.amount gt 0}">
								<bean:write name="model" property="bestPrice.displayAmount" filter="false" />
							</c:if>
						</td>
					</c:if>
				</tr>
		...
		<table cellspacing="0" class="form" summary="Subscription start date form.">
			<tr>
				...
				<td>
					<html:text styleClass="text" styleId="startDate" name="subscriptionForm" property="subscription.startDate.formDate" size="16" maxlength="16"/>										

					<script type="text/javascript">
						document.write('<a href="javascript:;" title="Date chooser"><img class="icon" src="../images/standard/icon/calendar.gif" alt="Date chooser" onclick="openDatePicker(document.getElementById(\'startDate\'))" width="16" height="15" style="padding-right: 0;" ><\/a>');
					</script>
