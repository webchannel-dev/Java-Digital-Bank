...
function uploadStart(fileObj) {
	try {
		...
		var progress = new FileProgress(fileObj, this.customSettings.progressTarget);
		progress.SetStatus(contentSnippets.uploading);
		progress.ToggleCancel(true, this);
		document.getElementById("flashTitle").innerHTML = "<h3>"+contentSnippets.uploadingWait+"</h3>";
		document.getElementById("filesUploading").style.display = "block";
...
function uploadComplete(fileObj, server_data) {
	//BB - not sure this is used any more. seem to use uploadCompleteHandler in flashuploader-js.jsp instead
	try {
		var progress = new FileProgress(fileObj, this.customSettings.progressTarget);
