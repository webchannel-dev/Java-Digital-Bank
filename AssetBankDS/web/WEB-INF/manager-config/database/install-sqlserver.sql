...
(
	...
	[IsSearchBuilderField] Tinyint Default 1 NOT NULL,
...
go
Alter table [BatchReleasedNotifications] add Constraint [BatchRelNot_Message_FK] foreign key([MessageId]) references [Message] ([Id])  on update no action on delete no action 
go
...
go
Alter table [BatchReleasedNotifications] add Constraint [BatchRelNot_BatchRel_FK] foreign key([BatchReleaseId]) references [BatchRelease] ([Id])  on update no action on delete no action 
go
...
go
Alter table [InDesignAsset] add Constraint [InDAsset_InDPDFStatus_FK] foreign key([PDFStatusId]) references [InDesignPDFStatus] ([Id])  on update no action on delete no action 
go
