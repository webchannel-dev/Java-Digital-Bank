...
<%@ taglib uri="/WEB-INF/bright-tag.tld" prefix="bright" %>
	...
	<c:if test="${attribute.autoComplete && attribute.isVisible}">
		if ($j('#field<bean:write name='attribute' property='id'/>').length>0) {
			...
			initAutocompleter("field<bean:write name='attribute' property='id'/>", "autocomplete_field<bean:write name='attribute' property='id'/>",delimiters,"<bean:write name='attribute' property='id'/>","<c:out value="${searchOrEdit}"/>");           
		}
	</c:if>
