...
<c:choose>
	...
	<c:otherwise>
		<c:set var="thumbSrc" value="../servlet/display?file=${asset.thumbnailImageFile.path}"/>
	</c:otherwise>
</c:choose>
...
<img class="<bean:write name='resultImgClass'/>" src="<bean:write name='thumbSrc'/>" alt="<bean:write name='asset' property='searchName'/>" <c:if test="${renderHover}">onmouseover="showPreview(this,'<bean:write name="asset" property="id"/>','<bean:write name="asset" property="previewImageFile.path"/>'); return false;" onmouseout="hidePreview('<bean:write name="asset" property="id"/>'); return false;"</c:if> />	
