	...
	<div id="navCol" class="clearfix">
		   ...
		   <c:if test="${(!extraContentAdminOnly || userprofile.isAdmin) && (!extraContentLoggedInOnly || userprofile.isLoggedIn)}">
				<c:if test="${not empty noOfExtraContentPages && noOfExtraContentPages > 0}">
					...
					<c:forEach begin="1" end="${noOfExtraContentPages}" var="index">
						...
						<bright:applicationSetting id="extraContentPageName" settingName='<%= "extra-content-page-name" + indexId %>'/>
					
						<li><a href="../action/viewContent?index=<c:out value='${index}'/>&showTitle=true" id="nav_content" <logic:equal name='section' value='<%= "extracontent" + indexId %>'>class="current"</logic:equal>><bean:write name="extraContentPageName" filter="false" /></a></li>		
		...
		<c:if test="${!userprofile.isFromCms && !hideLightbox}">
			...
			<logic:equal name="userprofile" property="isLoggedIn" value="true">
				...
				<c:choose>
					<c:when test="${userprofile.canApproveImageUploads || userprofile.isAdmin}">
						<li><a href="../action/viewAssetUploadApproval" id="nav_asset_approval" <logic:equal name='section' value='asset-approval'>class="current"</logic:equal>><bright:cmsWrite identifier="menu-approve" filter="false" case="mixed" /></a></li>
					</c:when>
					<c:otherwise>
						<c:if test="${userprofile.canApproveImages}">
			...
			<logic:equal name="userprofile" property="isLoggedIn" value="true">
            <c:if test="${userprofile.canUploadWithApproval || userprofile.canUpdateAssets}">
	            <li class="subholder"><a href="#" id="upload_nav_link" class="expanding"><bright:cmsWrite identifier="menu-upload" filter="false"/></a>
						<c:set var="showSubmitted" value="${usersWithUpdateCanAddAssetsToWF || userprofile.isAdmin || userprofile.canUploadWithApproval}"/>
						<ul id="upload_nav">
							...
							<li <c:if test="${!showSubmitted && !enableUnsubmittedAssets}">class="last"</c:if>><a href="../action/viewDataImport" id="nav_bulk_upload" <logic:equal name='section' value='import'>class="current"</logic:equal>><bright:cmsWrite identifier="menu-bulk" filter="false"/></a></li>
							<c:choose>
							...
							<!-- 
								<c:if test="${showSubmitted}">
									<li class="last"><a href="../action/viewOwnerAssetApproval" id="nav_submitted" <logic:equal name='section' value='view-submitted'>class="current"</logic:equal>><bright:cmsWrite identifier="menu-submitted" filter="false"/></a></li>				
								</c:if> 
								
								<li class="last"><a href="../action/viewMyUploads" id="nav_submitted" <logic:equal name='section' value='my-uploads'>class="current"</logic:equal>>My Uploads</a></li>	
							-->			
				...
				<c:if test="${userprofile.isAdmin || userprofile.isOrgUnitAdmin || userprofile.hasEditSubcategoryPermissions || (usingGroupRoles && reportAdmin)}">
   				...
   				<c:if test="${userprofile.isAdmin && assetEntitiesEnabled}">
					...
					<c:if test="${userprofile.isAdmin || userprofile.isOrgUnitAdmin || userprofile.canDeleteAssets}">	
         ...
         <%-- Light Box panel --%>
			<c:if test="${!userprofile.isFromCms && !hideLightbox && (!hideLightboxOnHomepage || section != 'home') && section != 'lightbox'}">
				...
				<div class="lbShadow">
					...
					<div class="lbPanel" id="lbPanel" style="margin-top:2em">
						...
						<div class="dropHolder singleButton">
							<h3><a href="../action/viewAssetBox" id="lbTitle" title="Go to this lightbox" ><c:if test="${userprofile.numAssetBoxes==1}"><bright:cmsWrite identifier="my-lightbox" filter="false"/></c:if><c:if test="${userprofile.numAssetBoxes>1}"><c:out value="${userprofile.assetBox.name}" /></c:if></a></h3>
							<c:if test="${multipleLightboxes && userprofile.isLoggedIn}">
								<a href="#" class="dropLink js-enabled-show" title="Switch lightbox">&nbsp;</a>
						...
						<c:if test="${multipleLightboxes && userprofile.numAssetBoxes>1}">
							...
							<form action="switchAssetBox" method="get" >
								<select style="margin-left: 10px; margin-top:0.6em; width:88px" name="currentAssetBoxId" id="selectLightbox" onchange="this.form.submit();">
									<logic:iterate name="userprofile" property="assetBoxes" id="assetBox">
										<option value="<bean:write name='assetBox' property='id'/>" <c:if test="${userprofile.assetBox.id==assetBox.id}">selected</c:if>><bean:write name='assetBox' property='name' filter='false'/></option>
									</logic:iterate>
								</select>
						...
						<p id="lbText">
							...
							<a title="Empty this lightbox" href="../action/clearAssetBox" class="empty <c:if test="${userprofile.assetBox.numAssets>0}">on</c:if>"><bright:cmsWrite identifier="snippet-empty"/></a>
