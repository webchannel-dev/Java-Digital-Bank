...
<c:if test="${not empty item.homogenizedImageFile.path}">
	&lt;img src=&quot;<c:out value="${appUrl}"/>/servlet/display?file=<c:out value="${item.homogenizedImageFile.path}"/>&quot; height=&quot;<c:out value="${thumbHeight}"/>&quot; width=&quot;<c:out value="${thumbWidth}"/>&quot; alt=&quot;<c:out value="${title}"/>&quot; /&gt;&lt;/a&gt;&lt;/p&gt;
</c:if>
<c:if test="${empty item.homogenizedImageFile.path}">
	&lt;img src=&quot;<c:out value="${appUrl}"/>/servlet/display?file=<c:out value="${item.thumbnailImageFile.path}"/>&quot; alt=&quot;<c:out value="${title}"/>&quot; /&gt;&lt;/a&gt;&lt;/p&gt;
</c:if>
<c:if test="${not empty description}">
	&lt;p&gt;<c:if test="${multiLineRssDescriptions}"><bright:writeWithCR name="description"/></c:if><c:if test="${!multiLineRssDescriptions}"><bean:write name="description"/></c:if>&lt;/p&gt;
		         ...
		         <media:content url="<c:out value="${appUrl}"/>/servlet/display?file=<c:out value="${item.previewImageFile.path}"/>" 
						       type="image/jpeg"
						       height=""
						       width=""/>
		        <media:title><c:out value="${title}"/></media:title>
		        <media:thumbnail url="<c:out value="${appUrl}"/>/servlet/display?file=<c:out value="${item.homogenizedImageFile.path}"/>" height="<c:out value="${thumbHeight}"/>" width="<c:out value="${thumbWidth}"/>" />
				</item>
			</logic:iterate>
		</logic:notEmpty>
	</channel>
