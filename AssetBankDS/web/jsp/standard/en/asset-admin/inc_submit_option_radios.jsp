							...
							<ul class="radioList" style="width:340px; <c:if test='${reposition}'>margin-top: -10px;</c:if>">
								...
								<logic:equal name="submitOptions" property="contains[4]" value="true">
									<li>
										<input type="radio" class="radio" id="submitoption_4<c:out value='${wfSuffix}'/>" name="selectedSubmitOption<c:out value='${wfSuffix}'/>" value="4" <c:if test="${selectedWorkflowLevel == null || selectedWorkflowLevel < 0 || selectedWorkflowLevel == 4}">checked</c:if>/>	
								...
								<logic:equal name="submitOptions" property="contains[0]" value="true">
									<li>
										<input type="radio" class="radio" id="submitoption_0<c:out value='${wfSuffix}'/>" name="selectedSubmitOption<c:out value='${wfSuffix}'/>" value="0" <c:if test='${wfSuffix != ""}'>onclick="checkUnsubmitted();"</c:if> <logic:present name='selectedWorkflowLevel'><c:if test="${selectedWorkflowLevel == 0}"> checked</c:if></logic:present><logic:notPresent name='selectedWorkflowLevel'><c:if test="${assetForm.selectedSubmitOption == 0}"> checked</c:if></logic:notPresent>/>	
										<span><label for="submitoption_0<c:out value='${wfSuffix}'/>"><bright:cmsWrite identifier="snippet-submit-option-live" filter="false"/>
										<c:if test="${submitOptions.hasMixedPermissions}"><em><bright:cmsWrite identifier="snippet-submit-option-live-if-permitted" filter="false"/></em></c:if></label></span>
									</li>
								</logic:equal>
								  
								<logic:equal name="submitOptions" property="contains[3]" value="true">
									<li>
										<input type="radio" class="radio" id="submitoption_3<c:out value='${wfSuffix}'/>" name="selectedSubmitOption<c:out value='${wfSuffix}'/>" value="3" <c:if test="${selectedWorkflowLevel == null || selectedWorkflowLevel < 0 || selectedWorkflowLevel == 3}">checked</c:if>/>	
								...
								<logic:equal name="submitOptions" property="contains[2]" value="true">
									<li>
										<input type="radio" class="radio" id="submitoption_2<c:out value='${wfSuffix}'/>" name="selectedSubmitOption<c:out value='${wfSuffix}'/>" value="2" <c:if test='${wfSuffix != ""}'>onclick="checkUnsubmitted();"</c:if><c:if test='${wfSuffix == ""}'>onclick="checkCanBeUnsubmitted(this);"</c:if> <logic:present name='selectedWorkflowLevel'><c:if test="${selectedWorkflowLevel == 2}"> checked</c:if></logic:present><logic:notPresent name='selectedWorkflowLevel'><c:if test="${assetForm.selectedSubmitOption == 2 || assetForm.asset.isUnsubmitted}"> checked</c:if></logic:notPresent>/>	
