...
<body id="detailsPage">
		...
		<c:if test="${assetForm.asset.hasFile || assetForm.asset.entity.id<=0 || assetForm.asset.entity.allowFiles || not empty assetForm.asset.entity.thumbnailFilename || !hidePreviewPanelIfNoFile}">
	    	<div class="floatLeft">
				...
				<c:choose>
					...
					<c:when test="${empty assetForm.asset.previewImageFile.path}">
						<div class="thumbFile">
	    	...
	    	<c:choose>
				...
				<c:otherwise>
		 		...
		 		<ul>		
					<%-- Button area --%>
					<%@include file="inc_view_asset_buttons.jsp"%>	
   ...
   <c:set var="fileSize" value="${assetForm.asset.fileSizeInBytes/(1024*1024)}"/>
	...
	<%-- If the asset doesn't have a file and has a type that a) can't have files and b) doesn't have a thumbnail, move the buttons here if hidePreviewPanelIfNoFile --%>
	<c:if test="${!(assetForm.asset.hasFile || assetForm.asset.entity.id<=0 || assetForm.asset.entity.allowFiles || not empty assetForm.asset.entity.thumbnailFilename || !hidePreviewPanelIfNoFile)}">
