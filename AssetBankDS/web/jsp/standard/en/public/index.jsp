...
<%@ page import="com.bright.assetbank.user.bean.ABUserProfile" %>
...
<c:set scope="session" var="imageDetailReturnName" value="Home"/>
...
<bright:applicationSetting id="accessLevelsOnHomepage" settingName="accessLevelsOnHomepage"/>	
...
<bright:applicationSetting id="showCategoriesOnQuickSearch" settingName="show-categories-on-quick-search"/>
		...
		<!--
			...
			$j(function () {
  				...
  				<c:if test="${autoCompleteEnabled}">
					...
					initAutocompleter("qkeywords","autocomplete_choices","");
				</c:if>
   ...
   <div id="rightCol">
   	<%@include file="../customisation/homepage_content.jsp"%>
		...
		<c:choose>
			<c:when test="${not empty homepageForm.featuredAsset}">
				...
				<div class="leftShadow">
					<div class="rightShadow">		
						<a href="../action/viewAsset?id=<bean:write name='homepageForm' property='featuredAsset.id'/>">
			...
			<c:when test="${empty homepageForm.featuredAsset && (categoryExplorerType == 'categories' || categoryExplorerType == 'accesslevels') && empty featuredCopyItem.body}">
				<div class="recent">
		...
		<c:if test="${numPromoted >0}">
			<div class="promo">
				<h3><bright:cmsWrite identifier="subhead-promoted-items" filter="false" /></h3>
				
				<logic:iterate name="homepageForm" property="promotedAssets" id="image" indexId="index">
					<a class="imgWrapper squareThumb<c:out value="${index+1}"/> <logic:empty name="image" property="homogenizedImageFile.path">iconWrapper</logic:empty>" href="../action/viewAsset?id=<bean:write name='image' property='id'/>">
				
						<logic:empty name="image" property="homogenizedImageFile.path">
							<bean:define id="asset" name="image" />
							<bean:define id="disablePreview" value="true"/>	
							<%@include file="../inc/view_thumbnail.jsp"%>	
						</logic:empty>
						<logic:notEmpty name="image" property="homogenizedImageFile.path">
							<bean:define id="asset" name="image" />
							<%@include file="../inc/view_homogenized_thumb.jsp"%>	
						</logic:notEmpty>
		...
		<logic:notEmpty name="homepageForm" property="recentImages">
			...
			<c:if test="${categoryExplorerType == 'none'}">
				<div class="recent">
					<h3><bright:cmsWrite identifier="subhead-recently-added" filter="false" /></h3>
					<logic:iterate name="homepageForm" property="recentImages" id="image" indexId="index">
						<a class="imgWrapper squareThumb<c:out value="${index+1}"/>" href="../action/viewAsset?id=<bean:write name='image' property='id'/>">
							<bean:define id="asset" name="image" />
							<%@include file="../inc/view_homogenized_thumb.jsp"%>	
	...
	<div id="leftCol">
		...
		<%@include file="inc_quick_search.jsp"%>
