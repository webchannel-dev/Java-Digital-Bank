...
<c:set var="heading"><bright:cmsWrite identifier="heading-least-downloaded" filter="false" case="mixed"/></c:set>

<%-- Set the return url and clear breadcrumb trail --%>
<c:set scope="session" var="breadcrumbTrail" value="" />	
<c:set scope="session" var="imageDetailReturnUrl" value="/action/viewLeastDownloadedAssets?page=${browseItemsForm.searchResults.pageIndex}&pageSize=${browseItemsForm.searchResults.pageSize}"/>
<c:set scope="session" var="imageDetailReturnName" value="${heading}"/>


<head>
	
	<title><bright:cmsWrite identifier="title-least-downloaded" filter="false" case="mixed"/></title> 
...
<body>
	...
	<div class="head <c:if test="${tabsPresent}">tabsAbove</c:if>">
   	...
   	<bright:cmsWrite identifier="heading-least-downloaded" filter="false" case="mixed"/>
   </div>

	<c:set var="formBean" value="${browseItemsForm}"/>
	<c:set var="linkUrl" value="viewLeastDownloadedAssets?t=1"/>
	<c:set var="styleClass" value="pager browsePager"/>
	<%@include file="../inc/pager.jsp"%>
	   	
	<h2><bright:cmsWrite identifier="heading-least-downloaded" filter="false" case="mixed"/></h2> 
	
	<!-- The intro text: this comes from the database (change in the 'Content' area of Admin) -->
	<bright:cmsWrite identifier="intro-least-downloaded" filter="false" />
	...
	<ul class="lightbox clearfix">
		...
		<logic:notPresent name="forwardParams">
			<c:set var="forwardParams">forward=/action/viewLeastDownloadedAssets<c:if test="${not empty p_page}">&page=<c:out value="${p_page}"/></c:if><c:if test="${not empty p_page}">&pageSize=<c:out value="${p_pageSize}"/></c:if></c:set>
		</logic:notPresent>
		
		<logic:iterate name='browseItemsForm' property='searchResults.searchResults' id='item' indexId='idx'>
			
			<c:set var="popularityId" value="4"/>
