			...
			<logic:iterate name="checkoutForm" property="approvalList" id="assetapproval" indexId="index">
				...
				<li class="clearfix">		
					...
					<c:choose>
							...
							<c:forEach var="price" items="${userprofile.assetBox.assetPrices[asset.id]}">
								...
								<span class="asset-price">	
									<c:choose>
										...
										<c:otherwise>
											<bean:write name='price' property='assetPrice.displayAmount' filter="false" />
										</c:otherwise>
									</c:choose>
								</span>	
								<br />
						...
						<c:otherwise>
							<bright:cmsWrite identifier="e-price" filter="false"/>: <bean:write name='asset' property='price.displayAmount' filter="false" /><br />				
						</c:otherwise>
					</c:choose>		
					</div>
				</li>		
