...
public class DetectVideoScenes extends Action{
    ...
    String taskName ="task_"+curTime; 
    String batchFilePath =AssetBankSettings.getApplicationPath()+"\\files\\temp\\";
    String batchFilename = batchFilePath+"task_"+curTime+".bat";
    //System.out.println(batchFilename);
    File batchFile = createTaskBatchFile(batchFilename,filePath);
    Calendar cal = Calendar.getInstance();
    cal.add(Calendar.MINUTE,1);
    DateFormat dateFormat = new SimpleDateFormat("HH:mm:ss");
    String taskStartTime = dateFormat.format(cal.getTime());
   // System.out.println(taskStartTime);
    String[] detectSceneCommand={"c:\\Windows\\System32\\schtasks.exe","/create","/tn", taskName, "/tr", batchFilename, "/sc", "once", "/st",taskStartTime,"/f"};
     try{
         int waitedFor = 1;
         ...
         do{
            Thread.sleep(60000L);
            ++waitedFor;
         }while(!isTaskCompleted(taskName) && waitedFor < 6);
         deleteTask(taskName);
         if(batchFile.exists())
             batchFile.delete();
      }
      catch (Bn2Exception be)
      {
          ...
          deleteTask(taskName);
           if(batchFile.exists())
             batchFile.delete();
          return mapping.findForward("failure");  // delete tasks here
      }
      ...
      try{
           ...
           //     System.out.println("calculatedstart :"+calculatedstar t+" tspan :"+tspan);
